...
<div>
    ...
    <div class="row hidden-print">
        ...
        <div class="col-lg">
            <div>
                <datepicker :disabledDates="disabledDates" v-model="start_date" :clear-button="true" placeholder="From Lead Date (dd/mm/yyyy)" format="dd/MM/yyyy" input-class="form-control" name="start_date"></datepicker>
            </div>
        </div>
        <div class="col-lg">
            <div>
                <datepicker :disabledDates="disabledDates" v-model="end_date" :clear-button="true"  placeholder="To Lead Date (dd/mm/yyyy)" format="dd/MM/yyyy" input-class="form-control" name="end_date"></datepicker>
            </div>
        </div>
            ...
            <div>
                ...
                <table class="myFormat table-hover" style="width: 100%;">
                    ...
                    <tr style="border-bottom: 2px solid black;" class="head">
                        ...
                        <td><STRONG>OPEN LEADS</STRONG></td>
                        <td><STRONG>IN-PROCESS LEADS</STRONG></td>
                        <td><STRONG>NO RESPONSE LEADS</STRONG></td>
                        <td><STRONG>MEMBERSHIPS DONE</STRONG></td>
                    </tr>
                    ...
                    <template v-for="(s,key) in  sliceP(
                     ...
                     return x;
                        ...
                        <tr>
                            ...
                            <td>{{s.assigned_to}}</td>
                            <td>{{s.username}}</td>
                            <td>{{s.leads | numFormat }}</td>
                            <td>{{s.open | numFormat }}</td>
                            <td>{{s.inprocess | numFormat }}</td>
                            <td>{{s.noresponse | numFormat }}</td>
                            <td>{{s.memberships | numFormat }}</td>
                        </tr>
                    ...
                    <tr style="border: 2px solid black;">
                        ...
                        <td>{{totalleads | numFormat}}</td>
                        <td>{{totalopen | numFormat}}</td>
                        <td>{{totalinprocess | numFormat}}</td>
                        <td>{{totalnoresponse | numFormat}}</td>
                        <td>{{totalmemberships | numFormat}}</td>
                    </tr>
    ...
    export default {
        ...
        methods:{
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/reports/lead-report/lead_init_vue?1=1'+x).then(result=>{
                    ...
                    if(!data.leads){
                        ...
                        x.forEach(function (s,key) {
                            a=a+parseInt(s.leads);
                            b=b+parseInt(s.open);
                            c=c+parseInt(s.inprocess);
                            d=d+(parseInt(s.noresponse));
                            e=e+(parseInt(s.memberships));

                        })
                        this.totalleads=a
                        this.totalopen=b
                        this.totalinprocess=c
                        this.totalnoresponse=d
                        this.totalmemberships=e
