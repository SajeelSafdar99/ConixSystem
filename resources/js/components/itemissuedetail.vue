            ...
            <table class="myFormat table-hover " style="width: 100%;">
                ...
                <template v-for="(s,key) in  sliceP(
                    ...
                    <tr v-if="(key==0) || sales[key-1].cat!=s.cat">
                        <td colspan="2" class="sub"><u style="text-transform: uppercase;">{{s.cat}}</u></td>
                    </tr>

                    <tr style="border-bottom: 2px solid black;" class="head" v-if="(key==0) || sales[key-1].cat!=s.cat">
                        <td class="text-uppercase"><STRONG>{{s.sub}}</STRONG></td>
                  ...
                  <tr v-if="sales[key+1]==undefined ||  sales[key+1].cat!=s.cat">
                      <td  style="text-transform: uppercase;  border-top: 1px solid #000!important;">&nbsp</td>
    ...
    export default {
        ...
        methods:{
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/reports/item-issue-detail/itemissuedetail_init_vue?1=1'+x).then(result=>{
                    let data=result.data;
                    if(!data.sales){
                        data.sales=[];
                    }
                    ...
                    if(this.getRe){
                        ...
                        x.forEach(function (s,key) {
                            ...
                            if(s.cat){
                                ee=ee+(parseInt(s.sub_total));
                            }
                            else{
                                ff=ff+(parseInt(s.sub_total));
                            }
