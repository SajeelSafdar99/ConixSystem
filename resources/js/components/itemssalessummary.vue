<template>
    <div>
        ...
        <!-- <div class="row hidden-print">
           ...
           &lt;!&ndash;  <select class="form-control " v-model="restaurant" name="restaurant[]" id="restaurant" multiple="multiple">
             ...
             <div class="col-lg">
                ...
                &lt;!&ndash; <select class="form-control" v-model="tabledef" name="tabledef[]" id="tabledef" multiple="multiple">
             ...
             <div class="col-lg">
                ...
                &lt;!&ndash; <select class="form-control" v-model="waiter" name="waiter[]" id="waiter" multiple="multiple">
     ...
     <br>
         <div class="row hidden-print">
             ...
             <div class="row mb-2">
                 ...
                 <input type="text" class="form-control" id="customer" v-model="customer" name="customer" autocomplete="off" placeholder="Search By Name..." value="">
             ...
             <div class="col-lg">
                     <p style="color: black;">Item Code:</p>
             ...
             <div class="col-lg">
                 <div>
                     ...
                     <datepicker v-model="start_date" :clear-button="true" format="dd/MM/yyyy" input-class="form-control" name="start_date"></datepicker>
             ...
             <div class="col-lg">
                 <div>
                     ...
                     <datepicker v-model="end_date" :clear-button="true" format="dd/MM/yyyy" input-class="form-control" name="end_date"></datepicker>
                 </div>
             </div>
...
<!--                         ITEM AND SUB-CAT FULL DEPENDENCY -->
...
<!--                        <div class="row">
            ...
            return x;
                        ...
                        <!--<br>
                        <div class="row">
                            <label class="col-sm-2 form-control-label">Error:</label>
                            <div class="col-sm-10">
                                <input type="checkbox" v-model="fFilter">
                            </div>
                        </div>-->
                        <br>
                        <div class="row">
        ...
        <div v-if="this.exported" class="hidden-print">
           <export-excel
            ...
            <div style="text-align: center; color: black; letter-spacing: 0.2em !important;">
                <h5>AFOHS <br> SALES SUMMARY (WITH ITEMS) </h5>
            </div>
                    ...
                    <div>
                     ...
                     return x;
                                ...
                                <tr v-if="((key+((page-1)*pagelength))==0) || sales[(key+((page-1)*pagelength))-1].cat!=s.cat" style="font-size: 5px !important;">
                                    <td colspan="2" style="text-transform: uppercase;"><b>{{s.cat}}</b> </td>

                                    <td  colspan="2" style="text-transform: uppercase;"><b>Date:</b> {{moment(s.dda).format('DD MMM YY')}}</td>

                                    <td colspan="4" style="text-transform: uppercase;"><b>Customer:</b> <template v-if="s.type==0">{{s.member}}</template><template v-else-if="s.type==1">{{s.guest}}</template><template v-else-if="s.type==3">{{s.employee}}</template></td>

                                    <td  colspan="3" style="text-transform: uppercase;"><b>Order Via:</b> {{s.ordertype}}</td>

                                </tr>

                                <tr v-if="((key+((page-1)*pagelength))==0) || sales[(key+((page-1)*pagelength))-1].cat!=s.cat">
                                    <td  colspan="2" style="text-transform: uppercase;"><b>Waiter:</b> {{s.waitor}}</td>
                                    <td  colspan="2" style="text-transform: uppercase;"><b>Table:</b> {{s.tables}}</td>
                                    <td colspan="4" style="text-transform: uppercase;"><b>Covers:</b> {{s.covers | numFormat }}</td>
                                    <td colspan="3" style="text-transform: uppercase;"><span v-if="fFilter"><b>Invoice Total:</b> <b>{{ (s.gtotal).toFixed(2)}}</b></span> </td>
                                ...
                                <!--<tr v-if="((key+((page-1)*pagelength))==0) || sales[(key+((page-1)*pagelength))-1].sub!=s.sub">

                                </tr>-->
                                <tr>
                                    <td>{{s.item_code}}</td>
                                    <td>{{s.item_details}}</td>
                                    <td>{{ parseFloat((s.sales).toFixed(2)).toLocaleString()}}</td>
                                    <td>{{s.sale_price | numFormat }}</td>
                                    <td>{{parseInt(s.sale_price2) | numFormat }}</td>
                                    <td>{{ parseFloat((((s.discount) * 100) / 100).toFixed(2)).toLocaleString() }}</td>
                                    <td>{{ parseFloat((((s.tax) * 100) / 100).toFixed(2)).toLocaleString() }}</td>
                                    <td>{{ parseFloat((((parseInt(s.sale_price2)-s.discount) * 100) / 100).toFixed(2)).toLocaleString() }}</td>
                                    <td>{{s.kot}}</td>
                                    <td>{{moment(s.wtime).format('hh:mm:ss A')}}</td>
                                  <!--  <td>{{s.status}}</td>-->
                                </tr>
                                ...
                                <tr v-if="sales[(key+((page-1)*pagelength))+1]==undefined ||  sales[(key+((page-1)*pagelength))+1].cat!=s.cat">
                                    ...
                                    <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{parseFloat((s.tsales).toFixed(2)).toLocaleString()}}</STRONG></td>
                                    ...
                                    <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat((((s.tdics) * 100) / 100).toFixed(2)).toLocaleString()}}</STRONG></td>
                                    <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{s.tax | numFormat }}</STRONG></td>
                                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat((((s.subtt-s.tdics) * 100) / 100).toFixed(2)).toLocaleString() }}</STRONG></td>
                            ...
                            <tr style="border: 2px solid black;">
                                ...
                                <td><STRONG>{{ parseFloat((stsales).toFixed(2)).toLocaleString()}}</STRONG></td>
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{ssubtt | numFormat }}</STRONG></td>
                                <td><STRONG>{{ parseFloat((((stdics) * 100) / 100).toFixed(2)).toLocaleString()}}</STRONG></td>
                                <td><STRONG>{{ parseFloat((((stax) * 100) / 100).toFixed(2)).toLocaleString() }}</STRONG></td>
                                <td><STRONG>{{ parseFloat((((ssubtt-stdics) * 100) / 100).toFixed(2)).toLocaleString() }}</STRONG></td>
                                <td><STRONG></STRONG></td>
                                <td><STRONG></STRONG></td>
                             <!--   <td><STRONG></STRONG></td>-->
                            </tr>
...
export default {
    ...
    methods:{
        ...
        init:function () {
            ...
            this.$http.get('/finance-and-management/reports/items-sales-summary/itemsummary_init_vue?1=1'+x).then(result=>{
                let data=result.data;
                if(!data.sales){
                    data.sales=[];
                }
                ...
                if(this.getRe){
                    ...
                    x.forEach(function (s,key) {
                        ...
                        b=b+parseFloat(s.sales);
                        c=c+parseFloat(s.discount);
                        d=d+(parseInt(s.sale_price2));
                        aa=aa+parseInt(s.sale_price);
                        bb=bb+parseFloat(s.sales);
                        cc=cc+parseFloat(s.discount);
                        ddd=ddd+parseInt(s.tax);
                        dd=dd+(parseInt(s.sale_price2));
                        aaa=aaa+parseInt(s.sale_price3);
                        bbb=bbb+parseFloat(s.sales);
                        ccc=ccc+parseFloat(s.discount);
                        dddd=dddd+(parseInt(s.sale_price2));
                ...
                if(this.fFilter){
                    ...
                    let sssc=this.pluck(data.sales.filter((a)=>{return a.itsalePrice!=null && Math.round(a.itsalePrice)!=Math.round(a.gtotal)}),'sale_id')
                    mxc=data.sales.filter((a)=>{return sssc.indexOf(a.sale_id)!=-1});
