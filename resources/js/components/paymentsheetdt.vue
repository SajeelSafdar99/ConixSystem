                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <br> <br>
                                    <input placeholder="Enter Remarks" class="form-control input-height" name="remarks" v-model="remarks" id="remarks">
                                    <br>
                                </div>
       ...
       <div class="row  hidden-print">
         ...
         <!--  <div class="col-lg">
               ...
               <input value="" class="form-control tablikebutton" type="number"
                      id="idd" v-model="idd"
                      name="idd" placeholder="Search by Expense ID">
           </div>-->
           ...
           <div class="col-lg">
            ...
            <div class="col-lg">
                ...
                <input value="" class="form-control tablikebutton" type="number"
                       id="expense_no" v-model="expense_no"
                       name="expense_no" placeholder="Search by Expense No.">
            </div>
            <div class="col-lg">
                <div>

                    <datepicker :disabledDates="disabledDates" v-model="start_date" :clear-button="true" placeholder="From Date (dd/mm/yyyy)" format="dd/MM/yyyy" input-class="form-control" name="start_date"></datepicker>
                </div>
            </div>
            <div class="col-lg">
                <div>

                   <datepicker :disabledDates="disabledDates" v-model="end_date" :clear-button="true" placeholder="To Date (dd/mm/yyyy)" format="dd/MM/yyyy" input-class="form-control" name="end_date"></datepicker>
                </div>
            </div>
           <div class="col-lg">
          ...
          <!-- <div class="col-lg">
               ...
               <multiselect track-by="name" label="name" placeholder="Choose Books" v-model="book" :multiple="true" :options="(()=>{let x=[];
            books.forEach((a)=>{
            ...
            <div>
                ...
                <table class="table-striped table-bordered  table-hover">
                    ...
                    <tr>
                        ...
                        <th class="wd-5p">EXPENSE NO</th>
                        <th class="wd-10p">EXPENSE DATE</th>
                ...
                sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                     ...
                     return x;
                        ...
                        <td>{{tr.expense_no}}</td>
                        <td>{{moment(tr.expense_date).format('DD/MM/YYYY')}}</td>
                        <td>{{tr.name}}</td>
                        <td>{{tr.supplier_id}}</td>
                       <!-- <td>{{tr.unit}}</td>
                        <td>{{tr.name}}</td>
                        <td>{{tr.amount}}</td>
                        <td>{{tr.status}}</td>-->
                        ...
                        <template v-else-if="(parseInt(tr.sumgrandtotal)-parseInt(tr.paid_amount))>0">
                            <template v-if="tr.supplier_id">
                                <td><button class="btn btn-sm btn-outline-danger active"><a style="color:white;" target="_blank" :href="'/finance-and-management/finance-payment-receipts/finance-payment-receipts-aeu'+'?'+'accid=' + tr.supplier_id">Unpaid</a></button></td>
                            </template>
                        ...
                        <td class="hidden-print"><button class="buttoncolor" title="View Documents"><a style="color:#000000;" target="_blank" :href="'/finance-and-management/finance-expenses/documents/' + tr.expense_no"><i class="fas fa-eye"></i></a></button></td>
                        </template>
                        <template v-else>
                            <td></td>
                        </template>

                        <td class="hidden-print"><button class="buttoncolor" title="Print"><a style="color:#000000;" target="_blank" :href="'/finance-and-management/finance-expenses/finance-expenses-invoice/' + tr.expense_no"><i class="fa fa-print" aria-hidden="true"></i></a></button></td>
                        <td class="hidden-print"><button class="buttoncolor" title="Edit"><a style="color:#000000;" target="_blank" :href="'/finance-and-management/finance-expenses/finance-expenses-aeu-vue/' + tr.expense_no"><i class="fas fa-edit"></i></a></button></td>
                        <td class="hidden-print"><button class="buttoncolor" @click="deleteme(tr.expense_no,tr.comments);" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
    ...
    computed: {
        totals() {
            ...
            x.forEach(function (item) {
                total=total + parseInt(item.amount?item.amount:0);
                sumgrandtotal=sumgrandtotal + parseInt(item.amount?item.amount:0);
    ...
    methods:{
     ...
     udf2(event){
        ...
        afterdel:function(){
            ...
            let url='/finance-and-management/payment-expenses/delete/'+this.deletethisid;
        ...
        filterData(expenses){
            ...
            if(this.accsearchid){
                if(this.accsearch!=''){
                    x=x.filter((a)=>{return  a.code?a.code.indexOf(this.accsearchid)!=-1:false});
                        //a.code==this.accsearchid
            ...
            if (this.searchId){
                ...
                x=x.filter((a)=>{return a.supplier_id==this.searchId});

            }

            if(this.expense_no){
                x=x.filter((a)=>{return a.expense_no==this.expense_no});
...
/*
            if(this.idd){
                x=x.filter((a)=>{return a.expense_id==this.idd});

            }*/

            if(this.start_date){
                x=x.filter((a)=>{return moment(a.expense_date,'YYYY-MM-DD').format('x')>=moment(moment(this.start_date).format('YYYY-MM-DD'),'YYYY-MM-DD').format('x')});
            }
            if(this.end_date){
                    x=x.filter((a)=>{return moment(a.expense_date,'YYYY-MM-DD').format('x')<=moment(moment(this.end_date).format('YYYY-MM-DD'),'YYYY-MM-DD').format('x')});
            }

          /*  if(this.book.length>0){
                x=x.filter((a)=>{return this.book.filter((m)=>{
                    return a.bookid==m.id;
                }).length>0});
