...
<!--                    <p style="color: black;">Card Status:</p>-->
                    <select v-model="cardstatus"  class="form-control">
                        <option v-for="s in ['All Cards','In-Process','Printed','Received','Issued','Re-Printed']">{{s}}</option>
                    </select>
                </div>
            </div>
                ...
                sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                    ...
                    <tr>
                        ...
                        <td><strong>{{totals.qtytotal | numFormat }} ({{totals.c | numFormat }})</strong></td>
                        <td><strong>{{totals.amount | numFormat }}</strong></td>
                        <td colspan="5"></td>
                    </tr>
                    </tfoot>
                </table>
    ...
    computed: {
        totals() {
            ...
            x.forEach(function (item) {
                ...
                qtytotal=qtytotal + parseInt(item.qty?item.qty:0);
    ...
    methods:{
        ...
        filterData(invoices){
            ...
            if(this.cardstatus){
                ...
                if(this.cardstatus=='In-Process'){
                    x=x.filter((a)=>{return a.cardstatus?a.cardstatus==this.cardstatus:a.cardstatus==this.cardstatus});

                }   else if(this.cardstatus=='Printed'){
                    x=x.filter((a)=>{return a.cardstatus?a.cardstatus==this.cardstatus:a.cardstatus==this.cardstatus});

                } else if(this.cardstatus=='Received'){
                    x=x.filter((a)=>{return a.cardstatus?a.cardstatus==this.cardstatus:a.cardstatus==this.cardstatus});

                }
                else if(this.cardstatus=='Issued'){
                    x=x.filter((a)=>{return a.cardstatus?a.cardstatus==this.cardstatus:a.cardstatus==this.cardstatus});

                }
                else if(this.cardstatus=='Re-Printed'){
                    x=x.filter((a)=>{return a.cardstatus?a.cardstatus==this.cardstatus:a.cardstatus==this.cardstatus});
        ...
        init:function () {
            ...
            this.$http.get('/sports-subscription/corporate-card-printing-datatable-ini').then(result=>{
