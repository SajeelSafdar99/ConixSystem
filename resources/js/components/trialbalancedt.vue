                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h5 class="modal-title">LEDGER ACCOUNTS:
                                            <a target="_blank" :href="'/finance-and-management/member-guest-ledgers-vue'+'?'+'start_date=' + this.ssdata.start_date+'?'+'end_date=' + this.ssdata.end_date+'?'+'mog=' + this.ssdata.mog+'?'+'searchId=' + this.ssdata.searchId+'?'+'filter=' + this.ssdata.filter+'?'+'ent=' + this.ssdata.ent">  <button type="button" title="Print"
...
<!--        <div class="row no-printme">
            <div class="col-sm-12">
                <div>
                    <button  v-on:click="sign=1" class=" btn btn-outline-primary disabled inactive btn-block mg-b-10 block">Total Debit: {{totals.debit | numFormat }}
                    </button>
                </div>
                <div>
                    <button v-on:click="sign=2"  class=" btn btn-outline-primary disabled inactive btn-block mg-b-10 block">Total Credit: {{totals.credit | numFormat }}
                    </button>
                </div>  <div>
                <button v-on:click="sign=0"  class=" btn btn-outline-primary disabled inactive btn-block mg-b-10 block">Total Balance: {{totals.credit -totals.debit | numFormat }}
                </button>
            </div>
            </div>
        </div>-->
       <!-- <div class="row no-printme">
        ...
        <div style="text-align: center; color: black;" class="print-only">
            <p><strong>( Date = Between {{this.start_date | moment("DD/MM/YYYY")}} To {{this.end_date | moment("DD/MM/YYYY")}}, Name = {{this.customer}}, Filter = {{this.filter}}, ENT = {{this.ent}} )</strong></p>
                ...
                sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                     ...
                     return x;
                        ...
                        <template v-else-if="tr.type==6">
                            <td><span class="text-success">(Corporate Member)</span></td>
                        </template>
                        <template  v-else-if="tr.type==3">
                            <td><span class="text-success">(Employee)</span></td>
                        </template>  <template  v-else-if="tr.type==2">
                            <td>(Supplier)</td>
                        </template>

                        <template v-if="tr.type==3">
                            <td>{{companies.filter((a)=>{return a.code==tr.company})[0]&&tr.company?companies.filter((a)=>{return a.code==tr.company})[0].name:''}}<br>{{tr.department?departments.filter((a)=>{return a.id==tr.department})[0].desc:''}}<br>{{tr.subdepartment?subdepartments.filter((a)=>{return a.id==tr.subdepartment})[0].desc:''}}</td>
                        </template>
                        <template v-else>
                            <td></td>
                        </template>
                        ...
                        <td>{{filters.filter((a)=>{return a.id==filter}).length>0?filters.filter((a)=>{return a.id==filter})[0].name:'All'}}</td>
                            ...
                            <td>{{ (tr.credit-tr.debit) | numFormat }}</td>
                    ...
                    <tr>
                        <td style="background-color: #1d91e2 !important" colspan="6" class="text-right"><strong>TOTAL : </strong></td>
...
export default {
    ...
    computed: {
        totals() {
            ...
            x.forEach(function (item) {
                ...
                debit=debit+parseInt(item.debit)
    ...
    methods:{
        ...
        filterData(trials){
            ...
            if (this.empId){
                x=x.filter((a)=>{return a.id==this.empId});
            }
            if (this.sign!=0){
        ...
        init:function () {
            ...
            this.$http.get('/finance-and-management/member-guest-trial-balance/trial_init_vue?start_date='+(this.start_date?moment(this.start_date).format('YYYY-MM-DD'):'')+'&end_date='+(this.end_date?moment(this.end_date).format('YYYY-MM-DD'):'')+'&filter='+this.filter+'&ent='+this.ent).then(result=>{
