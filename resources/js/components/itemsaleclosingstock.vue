                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <itemsaleclosingstockdetails :sdata="ssdata" v-if="ssdata" :key="'am'+k"></itemsaleclosingstockdetails>
                            </div>
...
<!--                    <th class="wd-10p">MANUFACTURER</th>-->
                 ...
                 <!--   <th class="wd-10p">STOCK</th>-->
                    <th class="wd-10p">TOTAL SALES</th>
                    <th class="wd-10p">TOTAL PURCHASES</th>

                    <th class="wd-10p">PRICE BALANCE</th>
...
<!--        <td>{{ parseFloat(parseFloat(tr.sales?tr.sales:0.0).toLocaleString() * parseFloat(tr.sqty?tr.sqty:0.0).toLocaleString()) }}</td>
                    <td>{{ parseFloat(parseFloat(tr.purchases?tr.purchases:0.0).toLocaleString() *parseFloat(tr.pqty?tr.pqty:0.0).toLocaleString())}}</td>
               -->
                    ...
                    <!--
                                        <template v-if="tr.in_or_out==1 && tr.type==1">
                                            <td>{{tr.purchases?tr.purchases:0}}</td>
                                        </template>
                                        <template v-else>
                                            <td>0</td>
                                        </template>
                                        <template v-if="tr.in_or_out==0 && tr.type==2">
                                            <td>{{tr.sales?tr.sales:0}}</td>
                                        </template>
                                        <template v-else>
                                            <td>0</td>
                                        </template>

                                        <td>{{ parseFloat(tr.in_or_out==1 && tr.type==1?tr.purchases:0)-parseFloat(tr.in_or_out==0 && tr.type==2?tr.sales:0)}}</td>



                                        <template v-if="tr.in_or_out==1 && tr.type==1">
                                            <td>{{tr.qty?tr.qty:0}}</td>
                                        </template>
                                        <template v-else>
                                            <td>0</td>
                                        </template>
                                        ...
                                        <td>{{ parseFloat(tr.in_or_out==1 && tr.type==1?tr.qty:0)-parseFloat(tr.in_or_out==0 && tr.type==2?tr.qty:0) }}</td>
                    -->
    ...
    export default {
        ...
        udf3(event){
            ...
            filterData(items){
             let   x=items;
               ...
               /* if (this.unitsearch){
                    ...
                    x=x.filter((a)=>{return a.unitcode?a.unitcode.split(',').indexOf(this.unitsearchid.toString())!=-1:false
                      });
                }*/
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/item-sale-closing-stock/init_vue?1=1'+d).then(result=>{
                    let data=result.data;
