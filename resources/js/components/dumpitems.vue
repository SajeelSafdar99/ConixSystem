...
-->
       ...
       <!-- <div class="row">
            <label class="col-sm-2 form-control-label">ITEM CODE:</label>
            <div class="col-sm-10">
        ...
        <br>
        <div class="row">
            <label class="col-sm-2 form-control-label">CANCELLED BY:</label>
            <div class="col-sm-10">
                <multiselect track-by="name" label="name" placeholder="Choose Options" v-model="cancelled_by" :multiple="true" :options="(()=>{let x=[];
            users.forEach((a)=>{
                x.push({name:a.name,id:a.id})
            })
            return x;
        ...
        <div>
            ...
            <table class="myFormat table-hover " style="width: 100%;">
                ...
                <tr style="border-bottom: 2px solid black;" class="head">
                    ...
                    <td><STRONG>FOOD VALUE</STRONG></td>
                ...
                <template v-for="(s,key) in  sliceP(
                  ...
                  <!--  <tr v-if="(key==0) || sales[key-1].sub!=s.sub">
                        <td colspan="12" class="cat"><u style="text-transform: uppercase;">{{s.sub}}</u></td>
                    </tr>
-->
                    ...
                    <tr>
                        ...
                        <td>{{(parseInt(s.sale_price)*parseFloat(s.sales)).toLocaleString() }}</td>
                        <td>{{s.cancelledby}}</td>
                       </tr>
                    <!--<tr v-if="sales[key+1]==undefined ||  sales[key+1].sub!=s.sub">
                        ...
                        <td></td>
                        <td></td>
                        <td><STRONG style="text-transform: uppercase;  border-top: 1px solid #000!important;">{{s.sub}} TOTAL:</STRONG></td>
                        <td><STRONG style="text-transform: uppercase;  border-top: 1px solid #000!important;">{{(s.tsales).toFixed(2)}}</STRONG></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td><STRONG style="text-transform: uppercase;  border-top: 1px solid #000!important;">{{s.tsalePrice | numFormat }}</STRONG></td>
                        <td><STRONG style="text-transform: uppercase;  border-top: 1px solid #000!important;">{{ (s.tsales*s.tsalePrice) | numFormat }}</STRONG></td>
                    </tr>-->
                ...
                <tr style="border: 2px solid black;">
                    ...
                    <td><STRONG>{{ parseFloat((ttsales).toFixed(2)).toLocaleString()}}</STRONG></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td><STRONG>{{ttprice | numFormat }}</STRONG></td>
                    <td><STRONG>{{ttvalue.toLocaleString() }}</STRONG></td>
                    <td></td>
                  </tr>
                </tbody>
        ...
        computed: {
            totals() {
                ...
                x.forEach(function (item) {
                    totalsales=totalsales + parseFloat(item.sales?item.sales:0);
                    totalsaleprice=totalsaleprice + parseInt(item.sale_price?item.sale_price:0);
                    })
                return {
        ...
        methods:{
            ...
            init:function () {
                ...
                if(this.waiter){
                    ...
                    x+='&mem='+this.mem_id_+'&mog='+this.mog;
                }
                if(this.mog){
                ...
                if(this.customer_id){
                    if(this.customer) {
                        x += '&mem=' + this.customer_id + '&mog=' + this.mog;
                    }
                } if(this.cate){
                ...
                if(this.cancelled_by){
                    x+='&cancelledby='+this.pluck(this.cancelled_by,'id').join(',');
                }
                if(this.start_date){
                ...
                this.$http.get('/finance-and-management/reports/daily-dump-items/dumpitems_init_vue?1=1'+x).then(result=>{
                    let data=result.data;
                    if(!data.sales){
                        data.sales=[];
                    }
                   ...
                   if(this.getRe){
                       ...
                       let cc=0;
                        ...
                        x.forEach(function (s,key) {
                            a=a+s.sale_price;
                            aa=aa+parseFloat(s.sales);
                            bb=bb+parseInt(s.sale_price);
                            cc=parseFloat(cc)+(parseInt(s.sale_price)*parseFloat(s.sales));
                            b=b+parseFloat(s.sales);
                            c=c+parseFloat(s.discount);
                            dS=dS+(parseInt(s.sale_price2));
                            d=d+(s.sale_price*s.sales);
