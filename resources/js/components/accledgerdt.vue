            ...
            <div class="col-lg">
                <div>
                    ...
                    <select class="form-control" v-model="pm" name="pm" id="pm">
                        <option value="0">All</option>
                        <option v-for="f in pms.filter((z)=>{return (this.accsearchid?this.accountss.filter((a)=>{return a.code==this.accsearchid})[0].cost_center:0)==(this.paymentmethods.filter((b)=>{return b.id==z.mod_id})[0].coa_trans_type)})" :value="f.id">
                            {{f.name}}
                        </option>
                    </select>
                </div>
            ...
            <div>
                ...
                <table class="table-striped table-bordered  table-hover">
                    ...
                    <tr>
                        ...
                        <th class="wd-10p">COA NAME</th>
                      <!--  <th class="wd-15p">TYPE</th>-->
                        <th class="wd-5p">ID</th>
                        <th class="wd-20p">NAME</th>
                        <th class="wd-15p">M / G TYPE</th>
                    ...
                    <tr style="background-color: #55bff9 !important;">
                        <td colspan="10" class="text-right"><STRONG>OPENING BALANCE: </STRONG></td>

                        <td class="text-left balance"><STRONG>{{opening | numFormat }}</STRONG></td>
                    </tr>
                    </thead>
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                     ...
                     return x;
                       ...
                       <!-- <td>{{tr.type_name}}</td>
                        ...
                        <template v-if="tr.type==1 || tr.type==0 || tr.type==2">
                           <td>{{tr.name}}</td>
                       </template>
                        <template  v-else-if="tr.type==3">
                                <td>{{tr.name}} ({{tr.company}} - {{tr.designation}})</td>
                            </template>
                        <template  v-else-if="tr.type==4">
                            <td>{{tr.coaname}}</td>
                        </template>
                        <template v-else>
                            <td></td>
                         ...
                         <template v-if="tr.type==1">
                            <td><span class="">{{tr.customer_no}} (Guest)</span></td>
                        </template>
                        <template v-else-if="tr.type==0">
                            <td><span class="">{{tr.mem_no}} (Member)</span></td>
                        </template>    <template v-else-if="tr.type==3">
                            <td><span class="">{{tr.emp_no}} (Employee)</span></td>
                        </template>
                        <template v-else-if="tr.type==2">
                            <td><span class="">{{tr.person_no}} (Supplier)</span></td>
                        </template>
                        <template v-else-if="tr.type==4">
                            <td><span class="">{{tr.trans_moc}} (COA ACCOUNT)</span></td>
                        </template>
                        <template v-else>
                            <td></td>
                        </template>
                        ...
                        <td>{{tr.receipt_id?tr.receipt_id:0}} - {{tr.type_name}} - (<template v-if="tr.debit_or_credit==1">Receipt</template><template v-else-if="tr.debit_or_credit==0">Payment</template><template v-else>None</template>)</td>
                        ...
                        <td>{{key==0?tr.b=tr.credit-tr.debit+parseInt(opening):tr.b=(cm[key-1].b)+(tr.credit-tr.debit) | numFormat }}</td>
...
export default {
    ...
    methods:{
        accountdata(){
            this.$http.post('/search/coa/cashbankdatalike',{searchid:this.accsearch}).then(result=>{
...
/*
        ...
        init:function () {
            ...
            if(!this.accsearch && !this.sdata){
                this.accsearchid='';
            }

                this.$http.get('/finance-and-management/accounts-ledgers/acc_init_vue?start_date='+(this.start_date?moment(this.start_date).format('YYYY-MM-DD'):'')+'&end_date='+(this.end_date?moment(this.end_date).format('YYYY-MM-DD'):'')+'&filter='+this.filter+'&moc='+this.mog+'&mocid='+this.searchId+'&pm='+this.pm+'&unitsearch='+this.unitsearchid+'&accsearchid='+this.accsearchid+'&ent='+this.ent).then(result=>{
                    let data=result.data;
                    this.ccs=data.ccs;
                    ...
                    this.accountss=data.accountss;
                    this.coaaccounts=data.coaaccounts;
                })
             }
            else{
                this.$snotify.error('Please select a COA !')
            }
/*
            let x='';
          if(this.filter){
    ...
    mounted() {
        if(this.sdata){
            ...
            this.pm=this.sdata.pm
            this.unitsearch=this.sdata.unitsearch
            this.accsearchid=this.sdata.account
    ...
    let partseven = uri[6].split("=");
    let parteight = uri[7].split("=");
    let partnine = uri[8].split("=");
    let partsix = uri[9].split("=");
        ...
        if(parteight[1]){
            this.unitsearch=parteight[1];
        }
        if(partnine[1]){
            this.account=partnine[1];
        }
        if(partsix[1]){
