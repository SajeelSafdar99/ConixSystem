...
-->
       ...
       <!-- <div class="row">
            <label class="col-sm-2 form-control-label">CATEGORY:</label>
            <div class="col-sm-10">
                <select v-model="cate"  class="form-control">
                    <option label="All" value="0"></option>
                    <option :value="s.id" v-for="s in categories">{{s.desc}}</option>
                </select>
            </div>
        </div>
        <br>
        <div class="row">
            ...
            <div class="col-sm-10">
                <select v-model="sub_cat"  class="form-control">
                    <option label="All" value="0"></option>
                    <option :value="sube.id" v-for="sube in subcategories">{{sube.desc}}</option>
                </select>
            </div>
        </div>
        <br>
        <div class="row">
            ...
            <div class="col-sm-10">
                <select v-model="item"  class="form-control">
                    <option label="All" value="0"></option>
                    <option :value="it.id" v-for="it in items">{{it.item_details}}</option>
                </select>
            </div>
        </div>-->
        <br>
...
<template v-if="showTable">
    ...
    <div style="text-align: center; color: black; letter-spacing: 0.2em !important;">
        <h5>AFOHS <br> DISH BREAKDOWN SUMMARY (PRICE)</h5>
    </div>
        ...
        <div>
            ...
            <table class="myFormat table-hover " style="width: 100%;">
                ...
                <template v-for="(s,key) in  sliceP(
                    ...
                    <tr v-if="(key==0) || sales[key-1].sub!=s.sub">
                        <td colspan="7" class="cat"><u style="text-transform: uppercase;">{{s.sub}}</u></td>
                    </tr>
                    <tr>
                        ...
                        <td>{{parseInt(s.sale_price)*parseInt(s.sales) | numFormat }}</td>
                        <td>{{ parseFloat((s.discount).toFixed(2)).toLocaleString()}}</td>
                        <td>{{ parseFloat((parseInt(s.sale_price2)-s.discount).toFixed(2)).toLocaleString()}}</td>
                    </tr>
                    <tr v-if="sales[key+1]==undefined ||  sales[key+1].sub!=s.sub">

                        <td></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{s.sub}} TOTAL:</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat(s.tsales?(s.tsales).toFixed(2):(s.tsales)).toLocaleString() }}</STRONG></td>
                        ...
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat((s.tdics).toFixed(2)).toLocaleString()}}</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat((s.subtt-s.tdics).toFixed(2)).toLocaleString()}}</STRONG></td>


                    </tr> <tr v-if="sales[key+1]==undefined ||  sales[key+1].cat!=s.cat">

                        <td></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{s.cat}} TOTAL:</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{ parseFloat((s.tsalesS).toFixed(2)).toLocaleString()}}</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG></STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{s.subttS | numFormat }}</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{parseFloat((s.tdicsS).toFixed(2)).toLocaleString()}}</STRONG></td>
                        <td style="text-transform: uppercase;  border-top: 1px solid #000!important;"><STRONG>{{parseFloat((s.subttS-s.tdicsS).toFixed(2)).toLocaleString()}}</STRONG></td>
                ...
                <tr style="border: 2px solid black;">
                    ...
                    <td><STRONG>{{ parseFloat((stdics).toFixed(2)).toLocaleString()}}</STRONG></td>
                    <td><STRONG>{{ parseFloat((ssubtt-stdics).toFixed(2)).toLocaleString()}}</STRONG></td>
                </tr>
                </tbody>
    ...
    export default {
        ...
        methods:{
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/reports/dish-breakdown-summary/dishbreakdownsummary_init_vue?1=1'+x).then(result=>{
                    let data=result.data;
                    if(!data.sales){
                        data.sales=[];
                    }
                    ...
                    if(this.getRe){
                        ...
                        x.forEach(function (s,key) {
                            ...
                            d=d+(parseFloat(s.sale_price2));
                            aS=aS+parseFloat(s.sale_price);
                            bS=bS+parseFloat(s.sales);
                            cS=cS+parseFloat(s.discount);
                            dS=dS+(parseFloat(s.sale_price2));
                            aa=aa+parseFloat(s.sale_price);
                            bb=bb+parseFloat(s.sales);
                            cc=cc+parseFloat(s.discount);
                            dd=dd+(parseFloat(s.sale_price2));
