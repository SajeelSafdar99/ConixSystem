             ...
             <div class="col-lg">
                <div>
                    ...
                    <select v-model="ent"  class="form-control">
                        <option v-for="s in ['Exclude Advance','Include Advance']">{{s}}</option>
                    </select>
                </div>
            </div>
            <div class="col-xs">
               ...
               cm = sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                     ...
                     return x;
                      ...
                      <!--  <template v-if="tr.debit_or_credit==0">
                            <td>{{tr.trans_type_id}}</td>
                        </template>
                        <template v-else-if="tr.debit_or_credit==1">-->
                            <td>{{tr.mainid}}</td>
                       <!-- </template>
                        <template v-else>
                            <td></td>
                        ...
                        <td>{{tr.receipt_id?tr.receipt_id:tr.trans_type_id}} - {{tr.type_name}} - (<template v-if="tr.debit_or_credit==1">Payment Voucher</template><template v-else-if="tr.debit_or_credit==0 && tr.maintype==6">Expense Bill</template><template v-else-if="tr.debit_or_credit==0 && tr.maintype==7">Payment Receipt</template><template v-else-if="tr.debit_or_credit==0 && tr.maintype==4">Purchase Bill</template><template v-else>None</template>)</td>
...
export default {
    ...
    methods:{
        ...
        init:function () {
            if(this.searchId){
                this.$http.get('/finance-and-management/supplier-ledgers/supplier_init_vue?start_date='+(this.start_date?moment(this.start_date).format('YYYY-MM-DD'):'')+'&end_date='+(this.end_date?moment(this.end_date).format('YYYY-MM-DD'):'')+'&filter='+this.filter+'&moc='+this.mog+'&mocid='+this.searchId+'&ent='+this.ent).then(result=>{
                ...
                this.$snotify.error('Please select a Supplier !')
            }
/*
            let x='';
          if(this.filter){
   ...
   //         this.init();
        ...
        if(partsix[1]){
             if(partsix[1]=='Exclude%20Advance'){
                 partsix[1]='Exclude Advance'
             }
            if(partsix[1]=='Include%20Advance'){
                partsix[1]='Include Advance'
            }
