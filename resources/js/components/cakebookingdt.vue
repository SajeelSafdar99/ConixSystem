                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h5 class="modal-title" style="color: black;">ARE YOU SURE ?</h5>
                                <button type="button" class="close" @click="CancelTheBooking=false">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">

                                <span style="color: black;">Do you want to Cancel this Booking ?</span>
                                <br> <br>
                                <input placeholder="Give Reason to Cancel" class="form-control input-height" v-model="reason" id="reason">

                                <br>

                            </div>
                            <div class="modal-footer">
                                <button @click="aftercancel(); CancelTheBooking=false;" class="btn btn-outline-warning active">Yes</button>
                                <button type="button" class="btn btn-secondary" @click="CancelTheBooking=false">Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="row hidden-print">
        ...
        <div class="col-lg">
            <div>
                ...
                <datepicker v-model="start_date" :clear-button="true" placeholder="dd/mm/yyyy" format="dd/MM/yyyy" input-class="form-control" name="start_date"></datepicker>
        ...
        <div class="col-lg">
            <div>
                ...
                <datepicker v-model="end_date" :clear-button="true" placeholder="dd/mm/yyyy" format="dd/MM/yyyy" input-class="form-control" name="end_date"></datepicker>
            </div>
        </div>

        <div class="col-lg">
            <div>
                <p style="color: black;">Delivery Date:</p>
                <datepicker v-model="delivery_date" placeholder="dd/mm/yyyy" :clear-button="true" format="dd/MM/yyyy" input-class="form-control" name="delivery_date"></datepicker>
            </div>
        </div>
...
-->
                ...
                sliceP(
                  ...
                  <!--  <td>{{tr.restaurant}}</td>
                    <td>{{tr.tabledef}}</td>
-->
                    <td>{{parseInt(tr.total_amount)  | numFormat }}</td>
                    <td>{{parseFloat(tr.discount) | numFormat }}</td>
                    <td>{{parseFloat(tr.tax) | numFormat }}</td>
                    <td>{{parseInt(tr.grand_total) | numFormat }}</td>
                   <!-- <td>{{ (tr.paid_amount?parseInt(tr.paid_amount):0) | numFormat }}</td>
                    ...
                    <td class="hidden-print"><button class="buttoncolor" title="View Documents"><a style="color:#000000;" target="_blank" :href="'/food-and-beverage/cake-booking/cake-booking-documents/' + tr.id"><i class="fas fa-eye"></i></a></button></td>
                    </template>
                    <template v-else>
                        <td></td>
                    </template>

                    <td class="hidden-print"><button class="buttoncolor" title="Print Invoice"><a style="color:#000000;" target="_blank" :href="'/food-and-beverage/cake-booking/cake-booking-invoice/' + tr.id"><i class="fa fa-print" aria-hidden="true"></i></a></button></td>
                    <td class="hidden-print"><button class="buttoncolor" @click="cancelme(tr.id);" title="Cancel"><i class="fas fa-times"></i></button></td>
                    <td class="hidden-print"><button class="buttoncolor" title="Edit"><a style="color:#000000;" target="_blank" :href="'/food-and-beverage/cake-booking/cake-booking-aeu-vue/' + tr.id"><i class="fas fa-edit"></i></a></button></td>
                    <td class="hidden-print"><button class="buttoncolor" @click="deleteme(tr.id,tr.note);" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
                </tr>
                ...
                <tr>
                    ...
                    <td><strong>{{(totals.tgross) | numFormat }}</strong></td>
                    <td><strong>{{(totals.tdisc) | numFormat }}</strong></td>
                    <td><strong>{{(totals.ttax) | numFormat }}</strong></td>
                    <td><strong>{{(totals.grand_total) | numFormat }}</strong></td>
                   <!-- <td><strong>{{(totals.paid_amount) | numFormat }}</strong></td>
                    <td><strong>{{ (totals.grand_total-totals.paid_amount) | numFormat }}</strong></td>
                   --> <td colspan="8"></td>
                </tr>
               ...
               <!-- <tr>
                    ...
                    <td><strong>{{toWords(totals.tsub)}}</strong></td>
    ...
    export default {
            ...
            totals() {
                ...
                //console.log(1);
                 x.forEach(function (item) {
                        tgross=tgross + parseInt(item.total_amount?item.total_amount:0);
                     tdisc=tdisc + parseFloat(item.discount?item.discount:0);
                     ttax=ttax + parseFloat(item.tax?item.tax:0);
        ...
        methods:{
            fup(){
                ...
                let url='/food-and-beverage/cake-booking/cancel/'+this.cancelthisid;
                if(this.validation(data,['reason'])==0){
            ...
            afterdel:function(){
                ...
                let url='/food-and-beverage/cake-booking/delete/'+this.deletethisid;
            ...
            filterData(sales){
             let   x=sales;
                if(this.booking_no){
                    x=x.filter((a)=>{return a.booking_no==this.booking_no});
                ...
                if(this.delivery_date){
                    x=x.filter((a)=>{return moment(a.delivery_date,'YYYY-MM-DD').format('x')==moment(moment(this.delivery_date).format('YYYY-MM-DD'),'YYYY-MM-DD').format('x')});
                }
                if(this.status){
            ...
            init:function () {
                ...
                this.$http.get('/food-and-beverage/cake-booking/cakebooking_init_vue').then(result=>{
