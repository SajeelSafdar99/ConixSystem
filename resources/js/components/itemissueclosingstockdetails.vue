...
<!--            {{subcats}}-->
        ...
        <div class="col-lg">
            ...
            <multiselect track-by="name" label="name" placeholder="Choose Store Locations" v-model="store_location" :multiple="true" :options="(()=>{let x=[];
            locations.forEach((a)=>{
                x.push({name:a.desc,id:a.id})
            })
            return x;
    ...
    <div style="text-align: center; color: black;" class="print-only">
        ...
        <p><strong>( Date = Between {{this.start_date | moment("DD/MM/YYYY")}} To {{this.end_date | moment("DD/MM/YYYY")}}, ID = {{this.searchid}}, Name = {{this.code}}, Category = {{this.pluck(this.category,'name')}}, Sub-Category = {{this.pluck(this.sub_category,'name')}}, Item = {{this.pluck(this.item_def,'name')}}, Store Location = {{this.pluck(this.store_location,'name')}}, Department = {{this.pluck(this.department,'name')}}, Company = {{this.unitsearchid}} )</strong></p>
...
<!--                    <th class="wd-10p">MANUFACTURER</th>-->
                 ...
                 <!--   <th class="wd-10p">STOCK</th>-->
                    <th class="wd-10p">TOTAL AMOUNT OUT</th>
                    <th class="wd-10p">TOTAL AMOUNT IN</th>

                    <th class="wd-5p">AMOUNT BALANCE</th>
...
<!--                    <td>{{tr.manufacturer}}</td>-->
                    ...
                    <td>{{tr.company}}</td>
                    <td>{{tr.store_location}}</td>
                    <td>{{tr.department}}</td>

                   <!-- <td>{{tr.opening_stock}}</td>-->
                    <template v-if="tr.in_or_out==0 && tr.type==2">
                        <td>{{ parseFloat(tr.price?tr.price:0).toLocaleString() }}</td>
                    </template>
                    <template v-else>
                        <td>0</td>
                    </template>
                    ...
                    <template v-if="tr.in_or_out==1 && tr.type==3">
                        <td>{{ parseFloat(tr.price?tr.price:0).toLocaleString() }}</td>
                    </template>
                    <template v-else>
                        <td>0</td>
                    ...
                    <td>{{key==0?tr.b=(parseFloat(tr.in_or_out==0 && tr.type==2?tr.price:0)-parseFloat(tr.in_or_out==1 && tr.type==3?tr.price:0)):tr.b=(cm[key-1].b)+(parseFloat(tr.in_or_out==0 && tr.type==2?tr.price:0)-parseFloat(tr.in_or_out==1 && tr.type==3?tr.price:0)) }}</td>
                    ...
                    <td>{{key==0?tr.b=(parseFloat(tr.in_or_out==1 && tr.type==3 && tr.qty?tr.qty:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.qty?tr.qty:0)):tr.b=(cm[key-1].b)+(parseFloat(tr.in_or_out==1 && tr.type==3 && tr.qty?tr.qty:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.qty?tr.qty:0)) }}</td>

                    <td>{{parseFloat(tr.in_or_out==0 && tr.type==2 && tr.price && tr.price!=0 && tr.qty?tr.price:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.pap && tr.price!=0 && tr.qty?tr.pap*tr.qty:0)}}</td>
                    <!--                    <td>{{tr.product}}</td>-->
...
<script>
    export default {
        name: "itemissueclosingstockdetails",
        json_data: [],
        props: ['sdata'],
        data(){
            return{
        ...
        computed: {
            totals() {
                ...
                x.forEach(function (item) {
                    ...
                    if(item.in_or_out==1 && item.type==3){
                        purchases=purchases+parseFloat(item.price?item.price:0)
                    }else if(item.in_or_out==0 && item.type==2){
                        sales=sales+parseFloat(item.price?item.price:0)
                    }
                    ...
                    if(item.in_or_out==0 && item.type==2){
                        profit=profit+(parseFloat(item.price?item.price:0)-parseFloat(item.pap?item.pap*item.qty:0))
                    }
        ...
        methods:{
            ...
            filterData(items){
             let   x=items;
                if(this.store_location.length>0){
                    x=x.filter((a)=>{return this.store_location.filter((m)=>{
                        return a.store_location==m.name;
                    }).length>0});
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/item-issue-closing-stock-details/init_vue').then(result=>{
                    let data=result.data;
