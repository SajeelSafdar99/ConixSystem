...
<!--                        <div class="col-sm-4 mg-t-10 mg-sm-t-0">
            ...
            <div class="col-lg">
                ...
                <input value="" class="form-control tablikebutton"  size="20" type="number"
                       ...
                       name="invoice_no" placeholder="Search by Invoice Id">
            </div>
            <div class="col-lg">
                <div>
                    <p style="color: black;">Begin Date:</p>
                    <datepicker v-model="start_date" :disabledDates="disabledDates" :clear-button="true" format="dd/MM/yyyy" input-class="form-control" name="start_date"></datepicker>
                </div>
            </div>
            <div class="col-lg">
                <div>
                    <p style="color: black;">End Date:</p>
                   <datepicker v-model="end_date" :disabledDates="disabledDates" :clear-button="true" format="dd/MM/yyyy" input-class="form-control" name="end_date"></datepicker>
                </div>
            </div>
        ...
        <div class="col-lg">
            <div>
                ...
                <select v-model="status"  class="form-control">
                    <option v-for="s in ['All','Male','Female','Other']">{{s}}</option>
                </select>
            </div>
        </div>
        <div class="col-lg">
        ...
        <div class="col-lg">
            ...
            <multiselect track-by="name" label="name" placeholder="Choose Cashiers" v-model="rcashier" :multiple="true" :options="(()=>{let x=[];
            users.forEach((a)=>{
                x.push({name:a.name,id:a.id})
            })
            return x;
...
<br>
        ...
        <div style="text-align: center; color: black;" class="print-only">
            <p><strong>( Date = Between {{this.start_date | moment("DD/MM/YYYY")}} To {{this.end_date | moment("DD/MM/YYYY")}},Type = <template v-if="this.mog>10">Guest</template><template v-else-if="this.mog==0">Member</template><template v-else-if="this.mog==6">Corporate Member</template><template v-else-if="this.mog==3">Employee</template><template v-else>All</template>, Name = {{this.customer}}, Invoice # = {{this.invoice_no}}, Details = {{this.details}}, Gender = {{this.status}}, Payment Method = {{this.pluck(this.payment,'name')}}, Receiving Cashiers = {{this.pluck(this.rcashier,'name')}} )</strong></p>
            ...
            <div>
                ...
                <table class="table-striped table-bordered  table-hover">
                    ...
                    <tr>
                        ...
                        <th class="wd-10p">SUBSCRIBER NAME</th>
                        <th class="wd-10p">MEMBER NAME</th>
...
<!--                        <td>{{moment(tr.invoice_date).format('DD/MM/YYYY')}}</td>-->
                        ...
                        <template v-if="tr.invoice_type==0">
                            <template v-if="tr.familia=='01'">
                                <td>Guest</td>
                            </template>
                            <template v-else-if="tr.familia=='02'">
                                <td>Self</td>
                            </template>
                            <template v-else-if="tr.fname!='   '">
                                <td>{{tr.fname}}</td>
                            </template>
                            <template v-else>
                                <td>{{tr.name}}</td>
...
-->                        <td>{{tr.tname}} <template v-if="tr.invoice_type==0">{{tr.mfgender?tr.mfgender:tr.mgender}}</template><template v-else-if="tr.invoice_type==6">{{tr.cmfgender?tr.cmfgender:tr.cgender}}</template></td>



                        <td v-if="tr.rname">{{tr.rname}}</td>
                        <td v-else>SELF</td>

                        <td>{{moment(tr.start_date).format('DD/MM/YYYY')}}</td>
                        <td>{{(moment(tr.end_date).format('DD/MM/YYYY'))}}</td>
...
export default {
    ...
    methods:{
            ...
            if(this.end_date){
                ...
                if(this.mog==6) {
                    if (this.status == 'Male') {
                        ...
                        x = x.filter((a) => {
                            return a.cmfgender ? a.cmfgender == this.status : a.cgender == this.status
                        });
                    ...
                    if (this.status == 'Male') {
                        ...
                        x = x.filter((a) => {
                            return a.mfgender ? a.mfgender == this.status : a.mgender == this.status
                        });
        ...
        init:function () {
            ...
            this.$http.get('/sports-subscription/subscriptions-datatable-ini').then(result=>{
