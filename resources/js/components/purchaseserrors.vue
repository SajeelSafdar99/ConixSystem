                    ...
                    <div>
                     ...
                     return x;
                                ...
                                <tr v-if="((key+((page-1)*pagelength))==0) || sales[(key+((page-1)*pagelength))-1].cat!=s.cat" style="font-size: 5px !important;">
                                    ...
                                    &nbsp&nbsp&nbsp&nbsp <b>Supplier:</b> {{s.sub}}</td>
    ...
    methods:{
        ...
        init:function () {
          ...
          if(this.specific){
            ...
            this.$http.get('/finance-and-management/reports/purchases-errors/purchaseserrors_init_vue?1=1'+x).then(result=>{
                let data=result.data;
                if(!data.sales){
                    data.sales=[];
                }
                ...
                if(this.fFilter){
                    let sssc=this.pluck(data.sales.filter((a)=>{return a.isubtt!=null && Math.round(a.isubtt)!=Math.round(a.gross)}),'purchase_id')
                    mxc=data.sales.filter((a)=>{return sssc.indexOf(a.purchase_id)!=-1});
                }
                    this.sales=mxc;
                    this.salesR=mxc;
