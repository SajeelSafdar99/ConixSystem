    ...
    methods:{
        ...
        init:function () {
          ...
          if(this.specific){
            ...
            this.$http.get('/finance-and-management/reports/store-sales-errors/storesaleserrors_init_vue?1=1'+x).then(result=>{
                let data=result.data;
                if(!data.sales){
                    data.sales=[];
                }
                ...
                if(this.fFilter){
                    let sssc=this.pluck(data.sales.filter((a)=>{return a.isubtt!=null && Math.round(a.isubtt)!=Math.round(a.gross)}),'sale_id')
