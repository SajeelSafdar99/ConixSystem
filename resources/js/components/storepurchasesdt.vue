...
<!--                    <td>{{parseInt(tr.sgross)}}</td>-->
                  ...
                  <td>{{tr.paid_amount?parseInt(tr.paid_amount):0}}</td>
                     ...
                     <template v-else-if="(parseInt(tr.grand_total-tr.paid_amount))>0">
                         <td><button class="btn btn-sm btn-outline-danger active"><a style="color:white;" target="_blank" :href="'/finance-and-management/finance-payment-receipts/finance-payment-receipts-aeu'+'?'+'accid=' + tr.customer_id">Unpaid</a></button></td>
                    </template>
                     <template v-else>
                    ...
                    <template v-if="tr.approved==0">
                        <td> <a style="color:white;" :href="'/store-management/store-purchases/approve/' + tr.id"> <button type="button" class="btn btn-sm btn-outline-danger active">Unapproved</button></a></td>
                    </template>
                    <template v-else>
                        <td> <a style="color:white;" :href="'/store-management/store-purchases/unapprove/' + tr.id"> <button type="button" class="btn btn-sm btn-outline-success active">Approved</button></a></td>
                    </template>


                    <td>{{tr.cashiername}}</td>
                    <template v-if="tr.image">
                        <td class="hidden-print"><button class="buttoncolor" title="View Documents"><a style="color:#000000;" target="_blank" :href="'/store-management/store-purchases/documents/' + tr.id"><i class="fas fa-eye"></i></a></button></td>
                    </template>
                    <template v-else>
                        <td></td>
                    </template>

                    <td class="hidden-print"><button class="buttoncolor" title="Print Invoice"><a style="color:#000000;" target="_blank" :href="'/store-management/store-purchases/store-purchases-invoice/' + tr.id"><i class="fa fa-print" aria-hidden="true"></i></a></button></td>

                    <template v-if="tr.approved==0">
                        <td class="hidden-print"><button class="buttoncolor" title="Edit"><a style="color:#000000;" target="_blank" :href="'/store-management/store-purchases/store-purchases-aeu/' + tr.id"><i class="fas fa-edit"></i></a></button></td>
                    </template>
                    <template v-else>
                        <td></td>
                    </template>
...
import Datepicker from 'vuejs-datepicker';
    export default {
        ...
        computed: {
            totals() {
                let  x=this.filterData(this.purchases);
        ...
        methods:{
            ...
            afterdel:function(){
                ...
                let url='/store-management/store-purchases/delete/'+this.deletethisid;
            ...
            init:function () {
                ...
                this.$http.get('/store-management/store-purchases/purchases_init_vue').then(result=>{
                    let data=result.data;
                    this.ccs=data.ccs;
                    this.purchases=data.purchases;
                    this.purchasesR=data.purchases;
                    this.leng=data.purchases.length;
                })
            },
