        ...
        <div>
            ...
            <table class="table-striped table-bordered table-hover ">
                ...
                <tr>
                    ...
                    <th class="wd-10p">ROOM CHARGES</th>
                    <th class="wd-10p">OTHER CHARGES</th>
                    <th class="wd-10p">FOOD BILLS TOTAL</th>
                    <th class="wd-10p">ADVANCE</th>
                    <th class="wd-5p">DISC</th>
                   <th class="wd-10p">INVOICE TOTAL</th>

                    <th class="wd-10p">AMOUNT PAYABLE</th>
                ...
                sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                    ...
                    <td>{{parseInt(tr.food_bills_total)}}</td>
                    <td>{{tr.advance_paid}}</td>


                    <td>{{(tr.discount_amount)}}</td>
                   <td>{{parseInt(tr.invoice_total)}}</td>
                    ...
                    <template v-else>
                        <td><button class="btn btn-sm btn-outline-danger active">Not Found</button></td>
                    </template>
...
<td>{{tr.cashiername}}</td>
                    ...
                    <td class="hidden-print"><button class="buttoncolor" title="View Documents"><a style="color:#000000;" target="_blank" :href="'/room-management/room-check-out/room-documents/' + tr.id"><i class="fas fa-eye"></i></a></button></td>
                    </template>
                    <template v-else>
                        <td></td>
                    </template>
                ...
                <tr>
                    ...
                    <td><strong>{{(totals.food_bills_total)}}</strong></td>
                    <td><strong>{{(totals.advance_paid)}}</strong></td>
                    <td><strong>{{(totals.discount_amount)}}</strong></td>
                   <td><strong>{{(totals.invoice_total)}}</strong></td>
               ...
               <!-- <tr>
                    ...
                    <td><strong>{{toWords(totals.charges)}}</strong></td>
                    <td><strong>{{toWords(totals.total_room_charges)}}</strong></td>
            ...
            totals() {
                ...
                //console.log(1);
                 x.forEach(function (item) {
                        ...
                        invoice_total=invoice_total + parseInt(item.invoice_total?item.invoice_total:0);
                        food_bills_total=food_bills_total + parseInt(item.food_bills_total?item.food_bills_total:0);
                     advance_paid=advance_paid + parseInt(item.advance_paid?item.advance_paid:0);
                ...
                return {
                    ...
                    food_bills_total:food_bills_total,
...
location.reload();
            ...
            init:function () {
                ...
                this.$http.get('/room-management/room-check-out/checkout-ini').then(result=>{
