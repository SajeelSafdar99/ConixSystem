...
<!--        <div class="col-sm">
                ...
                <input value="" class="form-control "  size="20" type="text"
                       ...
                       name="item_code" placeholder="Search By Item Code">
        </div>-->

        <div class="col-lg">
            <p>&nbsp</p>
            <div class="form-group" v-on:keydown.up.prevent="udf(1)" v-on:keydown.down.prevent="udf(0)">
            <input value="" type="text" class="form-control typeahead" autocomplete="off" id="code" v-model="code" name="code" placeholder="Search By Item Name...">
            <ul id="areabox" class="areabox" style="color: #fff;background: aliceblue;
        ...
        <div class="col-lg">
            ...
            <div class="form-group" v-on:keydown.up.prevent="udf3(1)" v-on:keydown.down.prevent="udf3(0)">
                <input value="" type="text" class="form-control typeahead" autocomplete="off" id="name" v-model="purchase_ref" name="name" placeholder="Search By Purchase Ref...">
                <ul id="areabox" class="areabox" style="color: #fff;background: aliceblue;
                list-style-type: none;color: black;" v-if="this.code && searcheditemsdefs.length>0">
...
<!--        <div class="col-lg">
            <div class="col-lg">
                &lt;!&ndash; <p style="color: black;">Category:</p>&ndash;&gt;
    ...
    <div style="text-align: center; color: black;" class="print-only">
        <p><strong>( Customer = {{this.customer}}, Date = Between {{this.start_date | moment("DD/MM/YYYY")}} To {{this.end_date | moment("DD/MM/YYYY")}}, ID = {{this.searchid}}, Name = {{this.code}}, Category = {{this.pluck(this.category,'name')}}, Sub-Category = {{this.pluck(this.sub_category,'name')}}, Item = {{this.pluck(this.item_def,'name')}}, Company = {{this.unitsearchid}}, Type = <template v-if="this.type==1">Sale</template><template v-else-if="this.type==2">Purchase</template><template v-else>Both</template> )</strong></p>
...
<!--                    <th class="wd-10p">MANUFACTURER</th>-->
                    ...
                    <th class="wd-25p">ITEM</th>
                    <th class="wd-10p">COMPANY</th>

                 <!--   <th class="wd-10p">STOCK</th>-->

                    <th class="wd-5p">TOTAL SALES</th>
                    <th class="wd-5p">TOTAL PURCHASES</th>

                    <th class="wd-5p">PRICE BALANCE</th>

                    <th class="wd-5p">TOTAL IN</th>
                    <th class="wd-5p">TOTAL OUT</th>

                    <th class="wd-5p">STOCK BALANCE</th>
                    <th class="wd-5p">PROFIT</th>
...
<!--                    <th class="wd-10p">STATUS</th>
             ...
             cm =   sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                    ...
                    <template v-if="tr.mogtype==0">
                        <td>{{tr.tname}} {{tr.fname}} {{tr.mname}} {{tr.lname}}</td>
                    </template>
                    <template v-else-if="tr.mogtype==1">
                        <td>{{tr.customer}}</td>
                    </template>
                    <template v-else-if="tr.mogtype==3">
                        <td>{{tr.employee}}</td>
                    </template>
                    <template v-else-if="tr.mogtype==2">
                    ...
                    <template v-if="tr.mogtype==1">
                        <template v-if="tr.cgt">
                            <td>{{tr.guesttype}} ({{tr.customerid}})</td>
                        </template>
                        <template v-else>
                            <td>Guest ({{tr.customerid}})</td>
                        </template>
                    </template>
                    <template v-else-if="tr.mogtype==3">
                        <td>Employee ({{tr.employeeid}})</td>
                    </template>    <template v-else-if="tr.mogtype==2">
                    <td>Supplier ({{tr.personid}})</td>
                </template>
                    <template v-else-if="tr.mogtype==0">
...
<!--                    <td>{{tr.manufacturer}}</td>-->
                   ...
                   <!-- <td>{{tr.opening_stock}}</td>-->
                    ...
                    <template v-if="tr.in_or_out==0 && tr.type==2">
                        <td>{{ parseFloat(tr.sale_price?tr.sale_price:0).toLocaleString() }}</td>
                    </template>
                    <template v-else>
                        <td>0</td>
                    </template>
                    <template v-if="tr.in_or_out==1 && tr.type==1">
                        <td>{{parseFloat(tr.purchase_price?tr.purchase_price:0).toLocaleString() }}</td>
                    </template>
                    <template v-else>
                        <td>0</td>
                    </template>
                    ...
                    <td>{{key==0?tr.r=parseFloat(parseFloat(tr.in_or_out==0 && tr.type==2 && tr.sale_price?tr.sale_price:0)-parseFloat(tr.in_or_out==1 && tr.type==1 && tr.purchase_price?tr.purchase_price:0)):tr.r=parseFloat((cm[key-1].r)+(parseFloat(tr.in_or_out==0 && tr.type==2  && tr.sale_price?tr.sale_price:0)-parseFloat(tr.in_or_out==1 && tr.type==1  && tr.purchase_price?tr.purchase_price:0))) }}</td>

                    <template v-if="tr.in_or_out==1 && tr.type==1">
                        <td>{{parseFloat(tr.qty?tr.qty:0).toLocaleString()}}</td>
                    </template>
                    <template v-else>
                        <td>0</td>
                    </template>
                    ...
                    <td>{{key==0?tr.b=(parseFloat(tr.in_or_out==1 && tr.type==1 && tr.qty?tr.qty:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.qty?tr.qty:0)):tr.b=(cm[key-1].b)+(parseFloat(tr.in_or_out==1 && tr.type==1 && tr.qty?tr.qty:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.qty?tr.qty:0)) }}</td>


                    <td>{{parseFloat(tr.in_or_out==0 && tr.type==2 && tr.sale_price && tr.sale_price!=0 && tr.qty?tr.sale_price:0)-parseFloat(tr.in_or_out==0 && tr.type==2 && tr.pap && tr.sale_price!=0 && tr.qty?tr.pap*tr.qty:0)}}</td>
<!--                    <td>{{tr.product}}</td>-->
...
<!--                   <template v-if="tr.activestatus==1">
               ...
               -->
                ...
                <tr>
                    <td style="background-color: #1d91e2 !important" colspan="8" class="text-right"><strong>TOTAL : </strong></td>
                    ...
                    <td style="background-color: #1d91e2 !important"><strong>{{ parseFloat((totals.profit).toFixed(1)).toLocaleString() }}</strong></td>
                </tr>
                </tbody>
...
<script>
    export default {
        name: "itemsaleclosingstockdetails",
        json_data: [],
        props: ['sdata'],
        data(){
            return{
        ...
        computed: {
            totals() {
                ...
                x.forEach(function (item) {
                    ...
                    if(item.in_or_out==1 && item.type==1){
                        purchases=purchases+parseFloat(item.purchase_price?item.purchase_price:0)
                    }else if(item.in_or_out==0 && item.type==2){
                        sales=sales+parseFloat(item.sale_price?item.sale_price:0)
                    }

                    if(item.in_or_out==1 && item.type==1){
                        ins=ins+parseFloat(item.qty?item.qty:0)
                    }else if(item.in_or_out==0 && item.type==2){
                        outs=outs+parseFloat(item.qty?item.qty:0)
                    }


                    if(item.in_or_out==0 && item.type==2){
                        profit=profit+(parseFloat(item.sale_price?item.sale_price:0)-parseFloat(item.pap?item.pap*item.qty:0))
                    }
            ...
            filterData(items){
             let   x=items;
                ...
                if(this.mgid){
                    x=x.filter((a)=>{
                        if(this.mog==1)
                            return a.customerid==this.mgid
                        else if(this.mog==2)
                            return a.personid==this.mgid
                        else if(this.mog==0)
                            return a.memid==this.mgid
                        else if(this.mog==3)
                            return a.employeeid==this.mgid
                        else if(this.mog==6)
                            return x=x;
                ...
                if(this.purchase_ref){
                    x=x.filter((a)=>{return a.purchase_ref==this.purchase_ref});
                }
                ...
                if(this.sdatacat){
                    x=x.filter((a)=>{return a.cate==this.sdatacat});
                }
                if(this.sdatasub){
                    x=x.filter((a)=>{return a.subcat==this.sdatasub});
                }
...
if(this.unitsearchid!=0){
    x = x.filter((a) => {
        return a.unitcode ? a.unitcode.split(',').indexOf(this.unitsearchid.toString()) != -1 : false
    });
}else{
    x=x;
}
...
*/
                ...
                if(this.type){
                    ...
                    if(this.type==1){
                        x=x.filter((a)=>{return a.in_or_out==0 && a.type==2});

                    }   else if(this.type==2){
                        x=x.filter((a)=>{return a.in_or_out==1 && a.type==1});
                    }
                    else{
                        x=x;
                    }
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/item-sale-closing-stock-details/init_vue').then(result=>{
                    let data=result.data;
        ...
        mounted() {
            ...
            if(this.sdata) {
                this.sdatacat = this.sdata.category
                this.sdatasub = this.sdata.sub_category
                this.searchid = this.sdata.searchid
                this.code = this.sdata.code
                this.item_code = this.sdata.item_code
                this.unitsearchid = this.sdata.unitsearchid
