...
<div v-if="this.exported" class="hidden-print">
    <export-excel
        ...
        name    = "IssueNoteSummaryDetail.xls">
    </export-excel>
</div>

    <div style="text-align: center; color: black; letter-spacing: 0.2em !important;">
        <h5>AFOHS <br>STORE ISSUE NOTE SUMMARY DETAIL </h5>
    </div>
...
<thead style="background-color: white;">
                ...
                <template v-for="(s,key) in  sliceP(
                    ...
                    <tr v-if="(key==0) || sales[key-1].sub!=s.sub">
                        <td colspan="2" class="cat"><u style="text-transform: uppercase;">{{s.sub}}</u></td>
                    </tr>
                    <tr>
    ...
    export default {
        ...
        methods:{
            ...
            init:function () {
                ...
                this.$http.get('/finance-and-management/reports/issue-note-summary-detail/issuesummarydetail_init_vue?1=1'+x).then(result=>{
                    let data=result.data;
                    if(!data.sales){
                        data.sales=[];
                    }
