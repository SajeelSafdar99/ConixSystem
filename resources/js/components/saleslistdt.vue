...
<!--            <div class="col-lg-3">
                <div class="row mb-2">
                 ...
                 &lt;!&ndash;   <div v-for="gt in gts" class="col-sm-3 mg-t-10 mg-sm-t-0">
                        <label class="rdiobox">
            ...
            <div class="col-lg">
                <div>
                    <select v-model="ent"  class="form-control">
                        <option v-for="s in ['Include ENT & CTS','Exclude ENT & CTS','Only ENT','Only CTS']">{{s}}</option>
                    </select>
                </div>
            </div>
<!--            <div class="col-lg">
            ...
            <div class="col-lg">
                <multiselect track-by="name" label="name" placeholder="Choose ENT Details" v-model="ent_detail" :multiple="true" :options="(()=>{let x=[];
            ent_details.forEach((a)=>{
                x.push({name:a.desc,id:a.id})
            })
            return x;
            })()"></multiselect>
                ...
                sliceP(
                  ...
                  if(parseInt(leng/pagelength)+((leng%pagelength)>0?1:0)<page){
                     ...
                     return x;
                        ...
                        <td>{{ (tr.grand_total?Math.round(tr.grand_total):0)-(tr.paid_amount?parseInt(tr.paid_amount):0) | numFormat }}</td>

                        <td style="color:#0053a7;">{{tr.reciept_id}}</td>
                        <td>{{tr.payment_method}}</td>
                        <template v-if="(parseInt(Math.round(tr.grand_total)-tr.paid_amount))<0">
                            <td><button class=" btn btn-sm btn-outline-warning active">Advance</button></td>
                        </template>
                        <template v-else-if="(parseInt(Math.round(tr.grand_total)-tr.paid_amount))>0">
                            <template v-if="tr.type==1">
                            ...
                            <template v-else-if="tr.type==6">
                                <td><button class="btn btn-sm btn-outline-danger active"><a style="color:white;" target="_blank" :href="'/finance-and-management/finance-cash-receipts/finance-cash-receipts-aeu'+'?'+'corporateid=' + tr.customer_id">Unpaid</a></button></td>
                            </template>
                            <template v-else-if="tr.type==3">
                        ...
                        <td><button class="buttoncolor" title="Show KOTs"><a style="color:#000000;" target="_blank" :href="'/food-and-beverage/sales-list-vue/kots/' + tr.id"><i class="fas fa-eye"></i></a></button></td>
                        ...
                        <!--                        <td class="hidden-print"><button class="buttoncolor" title="Edit"><a style="color:#000000;" target="_blank" :href="'/food-and-beverage/sales/sales-aeu/' + tr.id"><i class="fas fa-edit"></i></a></button></td>
                                                <td class="hidden-print"><button class="buttoncolor" title="Delete"><a style="color:#000000;" :href="'/food-and-beverage/sales/delete/' + tr.id"><i class="fa fa-trash" aria-hidden="true"></i></a></button></td>-->
                    </tr>
...
export default {
    ...
    methods:{
        ...
        init:function () {
            ...
            if(this.specific!=0){
                d=d+'&specific='+this.specific;
            }
            if(this.restaurant.length>0){
                d=d+'&resturants='+this.pluck(this.restaurant,'id').join(',');
            }
            if(this.tabledef.length>0){
                d=d+'&tables='+this.pluck(this.tabledef,'id').join(',');
            }
            if(this.waiter.length>0){
                d=d+'&waiter='+this.pluck(this.waiter,'id').join(',');
            }
            if(this.cashier.length>0){
            ...
            if(this.ent=='Exclude ENT & CTS'){
                d=d+'&ent='+'Exclude';
            }
            if(this.ent=='Only ENT'){
                d=d+'&ent='+this.ent;
            }
            if(this.ent=='Only CTS'){
                d=d+'&ent='+this.ent;
            }
            if(this.ent_detail.length>0){
                d=d+'&entdetail='+this.pluck(this.ent_detail,'id').join(',');
            }
            if(this.acftype!=0){
                d=d+'&acftype='+this.acftype;
            }

            if(this.tabledef.length>0 || this.waiter.length>0 || this.cashier.length>0 || this.acftype!=0 || this.ent!='Include ENT & CTS' || this.ent_detail.length>0 || this.start_date ||  this.end_date || this.booking_no || this.searchId || this.specific!=0 || this.mog!=2 || this.restaurant.length>0){
                d=d+'&kk='+1;
            }
            this.$http.get('/food-and-beverage/sales/sales_init_vue?1=1'+d).then(result=>{
    ...
    mounted() {
        document.addEventListener('keydown', (event)=> {
            if (event.key === 'Enter') {
                ...
                else if(this.tabledef.length>0 || this.waiter.length>0 || this.acftype!=0  || this.cashier.length>0 || this.ent!='Include ENT & CTS' || this.ent_detail.length>0 || this.start_date ||  this.end_date || this.booking_no || this.specific!=0  || this.restaurant.length>0){
                    this.init();
                    event.preventDefault();
                }
