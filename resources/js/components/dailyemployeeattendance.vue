        ...
        <div class="row  hidden-print">
            ...
            <div class="col-lg">
                ...
                <input value="" class="form-control tablikebutton"  size="20" type="number"
                       ...
                       name="employee_id" placeholder="Search by ID">
            </div>
...
<!--            <div class="col-lg">
        ...
        <div style="text-align: center; color: black; letter-spacing: 0.2em !important;">
            <h5>AFOHS <br>DAILY EMLOYEE ATTENDANCE SHEET </h5>

        </div>
        <div style="text-align: center; color: black;">
            <p><strong>Year = {{this.year}}, Month = {{this.month}}, ID = {{this.employee_id}}, Name = {{this.customer}}, Company = {{this.pluck(this.company,'name')}}, Department = {{this.pluck(this.department,'name')}}, Sub-Department = {{this.pluck(this.subdepartment,'name')}}</strong></p>
...
moment(end_date).diff(moment(start_date),'days')+1">
                            ...
                            <span v-for="(l,key) in  tr.visits.filter((a,k)=>{a.dur=diffCalc(a.in,a.out); return (moment(a.in).format('YYYY-MM-DD')==moment(start_date).add(n-1,'days').format('YYYY-MM-DD') || moment(a.out).format('YYYY-MM-DD')==moment(start_date).add(n-1,'days').format('YYYY-MM-DD'))})">

                              <span  :title="l.out?'Total Working Hours: '+l.dur:''">
                                <span :style="'display:block;margin-bottom:5px;border:2px solid green'" v-if="moment(l.in).format('YYYY-MM-DD')==moment(start_date).add(n-1,'days').format('YYYY-MM-DD')">
                                 {{moment(l.in).format('hh:mm A')}}

                                  </span>
                                  <template v-if="moment(l.out).format('YYYY-MM-DD')==moment(start_date).add(n-1,'days').format('YYYY-MM-DD')">
                                <span :style="'display:block;margin-bottom:5px;border:2px solid red'" >
                                 {{moment(l.out).format('hh:mm A')}}
...
// duration in minutes
        ...
        init:function () {
            ...
            this.$http.get('/human-resource/employment/daily_init_vue?1=1'+params).then(result=>{
                ...
                this.employees.forEach((x)=>{
                    if(x.visits){
                        x.totd=x.visits.length;
                      //  console.log(this.pluck(x.visits.filter((a)=>{a.ddf=this.diffCalc(a.in,a.out); return true}),'ddf'))
                        x.toth=this.calctime(this.pluck(x.visits.filter((a)=>{a.ddf=this.diffCalc(a.in,a.out);return true}),'ddf'));
                        x.tota=moment(this.end_date).diff(moment(this.start_date),'days')-x.totd;
                    }
                    else{
                        x.totd=0;
                        x.tota=moment(this.end_date).diff(moment(this.start_date),'days');
