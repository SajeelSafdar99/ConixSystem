...
export default {
    ...
    computed: {
           /* totals() {
                ...
                let mat =this.start_date;
                let  d=this.filterData(this.employees);

                let  x=moment(this.end_date).diff(moment(this.start_date),'days')+1;

                d.forEach(function (sdd) {
                    ...
                    let  s=sdd.foodbills.filter((a,k)=>{  return (moment(a.date).format('YYYY-MM-DD')==moment(mat).add(x[i]-1,'days').format('YYYY-MM-DD'))});
                    s.forEach(function (kiki) {


                    tgross=tgross + parseInt(kiki.ddf?kiki.ddf:0);
                    console.log(tgross);
...
// duration in minutes
        ...
        init:function () {
            ...
            this.$http.get('/finance-and-management/reports/totalfoodbills_init_vue?1=1'+params).then(result=>{
