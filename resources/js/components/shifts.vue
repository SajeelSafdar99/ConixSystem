                    ...
                    <div class="modal-wrapper">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <input @click="end_shift();"  :class="' btn btn-success'" type="button" :value="'YES'">
                    ...
                    <div class="modal-wrapper">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <input @click="start_shift(); "  :class="' btn btn-success'" type="button" :value="'YES'">
                                    <a href="/food-and-beverage" ><button type="button" class="btn btn-danger">NO</button></a>
                                </div>
                            </div>
                        </div>
...
'    height: 61px;margin-top:'+tableMargin+'px'">
                                                <div class="scrollsubrests" v-for="table in tables" >
                                                    ...
                                                    <template v-if="bookedTables.length>0 && bookedTables.indexOf(String(table.id))!=-1">
                                                        <div class="tabledefs booked" @click="sTable=table.id;"
                                                    ...
                                                    <template v-else-if="printedTables.length>0 && printedTables.indexOf(String(table.id))!=-1">
                                                        <div class="tabledefs printed" @click="sTable=table.id;"
                                                             v-bind:class="{'active':sTable==table.id}">
            ...
            <div class="form-layout form-layout-4 blackcolor">
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <label class="rdiobox">
                            <input type="radio" name="type" v-model="type" value="1"><span class="pabs">Guest   <a href="/room-management/room-customer/room-customer-aeu" target="_blank" class="btn btn-xsss btn-info"><i class="fa fa-plus"></i></a></span>
                        </label>
                        <label class="rdiobox">
                ...
                <div class="row finishbuttons">
                    <div class="col-sm-12">
                        <template v-if="(this.printedornot!=2) && this.caneditsale">
                            <input @click="save_sales(id)"  :class="id?'btn-info':'btn-info'" type="submit" name="save" class="btn" :value="id?'Edit Order':'Save Order'">
                        ...
                        <a href="/food-and-beverage/sales-vue" target="_blank"><button class="btn btn-secondary">Re-Issue Bill</button></a>
...
window.location.href='/food-and-beverage/sales/sales-aeu?t='+tableid;*/
            ...
            if(this.validation(data, ['customer', 'customer_id', 'sResturant', 'sTable', 'sType', 'selected_items','sAccType']) == 0)
    {
        this.$http.post(url, data).then(result => {
            ...
            update_movetable:function(m){
                ...
                if(this.validation(data,['sResturant','sTable'])==0){
                    this.$http.post(url,data).then(result=>{
            ...
            save_sales:function(m){
                let data={
                    selected_items:this.selected_items,
                    date:this.shift[0]?this.shift[0]:'',
                    shift:this.shift,
                    time:moment().format("h:mm:ss A"),
                    customer:this.customer,
                    type:this.type,
...
if(this.validation(data,['date', 'time','shift','customer' ,'customer_id','sResturant','sTable','sType','selected_items'])==0){
    this.$http.post(url,data).then(result=>{
