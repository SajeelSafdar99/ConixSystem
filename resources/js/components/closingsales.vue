     ...
     -->
        <template v-if="showFilters">
            ...
            return x;
                        ...
                        <div class="row">
                            <label class="col-sm-2 form-control-label">ORDER TYPE:</label>
                            <div class="col-sm-10">
                                <multiselect track-by="name" label="name" placeholder="Choose Options" v-model="order_type" :multiple="true" :options="(()=>{let x=[];
            ['Dine-In','Take Away','Home Delivery'].forEach((a)=>{
                x.push({name:a})
            })
            return x;
            })()"></multiselect>
                    ...
                    <div>
                     ...
                     return x;
                                ...
                                <tr style="border-bottom: 2px solid black;" class="head" v-if="(key==0)">
                                    ...
                                    <td><STRONG>TOTAL COVERS</STRONG></td>
<!--                                    <template v-for="ee in ent_details">
                                        <td ><STRONG>{{ee.desc}} ENT</STRONG></td>
                                    </template>-->
                                </tr>
                                ...
                                <tr v-if="(key==0) || sales[key-1].cat!=s.cat">
                                    <td colspan="15" class="sub"><u style="text-transform: uppercase;">{{s.cat}}</u></td>
                                    <template v-for="ee in ent_details">
                                        <td>&nbsp</td>
                                    </template>
                                </tr>
...
<td></td>
                                    ...
                                    <td>{{ parseFloat((s.diss).toFixed(1)).toLocaleString() }}</td>
                                   <td>{{ parseFloat((s.netsale).toFixed(1)).toLocaleString() }}</td>
                                   <td>{{s.taxx | numFormat }}</td>

                                    <td>{{ parseFloat((s.cashgross).toFixed(1)).toLocaleString() }}</td>
                                    <td>{{ parseFloat((s.creditgross).toFixed(1)).toLocaleString() }}</td>
                                    <td>{{ parseFloat((s.othergross).toFixed(1)).toLocaleString() }}</td>
                                   <td>{{s.paid_amount | numFormat }}</td>
                                   <td>{{ (parseInt(s.grandd)-parseInt(s.paid_amount)) | numFormat }}</td>
                                   <td><strong >{{ parseFloat((s.grandd).toFixed(1)).toLocaleString() }}</strong></td>
                                   <td>{{s.netcovers | numFormat }}</td>

<!--                                   <template v-for="ee in ent_details" >
                                       <td v-if="ee.id==1">{{ (s.grandentone).toFixed(1) }}</td>
                                       <td v-if="ee.id==2">{{ (s.grandenttwo).toFixed(1) }}</td>
                                       <td v-if="ee.id==3">{{ (s.grandentthree).toFixed(1) }}</td>
                                       <td v-if="ee.id==4">{{ (s.grandentfour).toFixed(1) }}</td>
                                       <td v-if="ee.id==5">{{ (s.grandentfive).toFixed(1) }}</td>
                                   </template>-->
                                </tr>
                          ...
                          <tr style="border-top: 2px solid black;">
                                ...
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{ parseFloat((bb).toFixed(1)).toLocaleString() }}</STRONG></td>
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{dd | numFormat }}</STRONG></td>
                                <td><STRONG></STRONG></td>
                              <td><STRONG>{{parseFloat((gg).toFixed(1)).toLocaleString() }}</STRONG></td>
                              <td><STRONG></STRONG></td>
                              <td><STRONG>{{jj | numFormat }}</STRONG></td>
                              <td><STRONG></STRONG></td>
                              <td><STRONG>{{parseFloat((ee).toFixed(1)).toLocaleString() }}</STRONG></td>
                              <td><STRONG></STRONG></td>
<!--                              <template v-for="ee in ent_details" >
                                  <td v-if="ee.id==1"><STRONG>{{ (kk).toFixed(1) }}</STRONG></td>
                                  <td v-if="ee.id==2"><STRONG>{{ (ll).toFixed(1) }}</STRONG></td>
                                  <td v-if="ee.id==3"><STRONG>{{ (mm).toFixed(1) }}</STRONG></td>
                                  <td v-if="ee.id==4"><STRONG>{{ (nn).toFixed(1) }}</STRONG></td>
                                  <td v-if="ee.id==5"><STRONG>{{ (oo).toFixed(1) }}</STRONG></td>
                              </template>-->
                            </tr>
                            <tr style="border-bottom: 2px solid black;">
                                ...
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{ parseFloat((cc).toFixed(1)).toLocaleString() }}</STRONG></td>
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{ parseFloat((ff).toFixed(1)).toLocaleString() }}</STRONG></td>
                                <td><STRONG></STRONG></td>
                                <td><STRONG>{{ parseFloat((hh).toFixed(1)).toLocaleString() }}</STRONG></td>
                                <td><STRONG></STRONG></td>
...
export default {
    ...
    methods:{
        ...
        init:function () {
            ...
            this.$http.get('/finance-and-management/reports/closing-sales-report/closingsales_init_vue?1=1'+x).then(result=>{
                let data=result.data;
                if(!data.sales){
                    data.sales=[];
                }
                ...
                if(this.getRe){
                    ...
                    x.forEach(function (s,key) {
                        ...
                        d=d+(parseInt(s.taxx));
                        e=e+(parseFloat(s.grandd?s.grandd:0));
                        f=f+(parseFloat(s.cashgross));
                        g=g+(parseFloat(s.creditgross));
                        h=h+(parseFloat(s.othergross));
                        i=i+(parseInt(s.netcovers?s.netcovers:0));
                        aa=aa+parseInt(s.grosssale);
                        bb=bb+parseFloat(s.diss);
                        cc=cc+parseFloat(s.netsale);
                        dd=dd+(parseInt(s.taxx));
                        ee=ee+(parseFloat(s.grandd?s.grandd:0));
                        kk=kk+(parseFloat(s.grandentone?s.grandentone:0));
                        ll=ll+(parseFloat(s.grandenttwo?s.grandenttwo:0));
                        mm=mm+(parseFloat(s.grandentthree?s.grandentthree:0));
                        nn=nn+(parseFloat(s.grandentfour?s.grandentfour:0));
                        oo=oo+(parseFloat(s.grandentfive?s.grandentfive:0));

                        ff=ff+(parseFloat(s.cashgross));
                        gg=gg+(parseFloat(s.creditgross));
                        hh=hh+(parseFloat(s.othergross));
                        ii=ii+(parseInt(s.netcovers?s.netcovers:0));
