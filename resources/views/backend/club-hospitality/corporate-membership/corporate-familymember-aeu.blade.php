...
<ul class="breadcrumbee mg-b-25 border-bottom-custom">
  ...
  <li><a href>Edit Corporate Family Member</a></li>
</ul>
@else
<ul class="breadcrumbee mg-b-25 border-bottom-custom">
  <li><a href="{{ url('/') }}">Home</a></li>
  <li><a href="{{ url('club-hospitality') }}">Club Membership Management</a></li>
  <li><a href="{{ url('club-hospitality/corporate-membership-vue') }}">Corporate Memberships List</a></li>
  <li><a href>Add Corporate Family Member</a></li>
</ul>
@endif
...
<div class="w3-bar w3-black">
   @if($init==1)
   <a href="{{ url('club-hospitality/corporate-membership/corporate-membership-aeu') }}/{{Request::segment(4)}}">
  <button class="w3-bar-item w3-button  w3-red ">Corporate Member</button>
</a>
  @else
 <button class="w3-bar-item w3-button w3-red " onclick="location.href='{{ url('club-hospitality/corporate-membership/corporate-membership-aeu/') }}/{{Request::segment(4)}}' ">Corporate Member</button>
  @endif


  <button class="w3-bar-item w3-button  w3-red theactiveclass" onclick="location.href='{{ url('club-hospitality/corporate-membership/corporate-familymember-aeu/') }}/{{Request::segment(4)}}' ">Family Member</button>


   @can('View Corporate Membership Documents')
    <button class="w3-bar-item w3-button w3-red" onclick="location.href='{{ url('club-hospitality/corporate-membership/corporate-membership-docs-aeu/') }}/{{Request::segment(4)}}' ">Membership Documents</button>
    @endcan
 

  @can('View Corporate Cards')
  <a href="{{ url('corporate-memberprint') }}/{{Request::segment(4)}}" target="_blank">
  <button class="w3-bar-item w3-button  w3-red">Card</button>
</a>
  @endcan

   @can('View Corporate Cards')
  <a href="{{ url('club-hospitality/corporate-membership/familymembercard') }}/{{Request::segment(4)}}">
...
<div style="text-align: right !important; margin-top: -50px;" class="col-sm-8 mg-t-10 mg-sm-t-0">
          @can('Export Corporate Family Member Columns')
             <button type="button" data-toggle="modal" data-target="#modal" class="mg-t-30 btn btn-primary"><i class="fa fa-file"></i> Export</button>
          @endcan

          @can('View Deleted Corporate Family Members')
          <a href="{{ url('club-hospitality/corporate-familymember/deleted') }}/{{$membershipdata->id}}">
...
@if($init==1)
 @can('Edit Corporate Family Member')
  ...
  color: #fff;
    ...
    @can('Add Corporate Family Member')
...
<div id="modal" class="modal fade" role="dialog">
                <div class="modal-dialog modal-lg">
                    ...
                    <div class="modal-content">
                        <form method="post" action="{{route('corporate-familymember.export')}}">
                            {{csrf_field()}}
              ...
              <thead>
                <tr>
                  ...
                  <th class="wd-10p">Barcode #</th>
                   ...
                   @endcan
                   @can('Delete Corporate Family Member')
...
{{-- Family Member Modal --}}
     ...
     @if($init==1 && Gate::check('Update Corporate Supplementary Barcode'))
   <form method="post" action="{{ url('club-hospitality/corporate-familymember/barcodeupdate/') }}/{{ $family_update->member_id }}/{{ $family_update->id }}" enctype="multipart/form-data">
      @elseif($init==1 && !Gate::check('Update Corporate Supplementary Barcode'))
 <form method="post" action="{{ url('club-hospitality/corporate-familymember/update/') }}/{{ $family_update->member_id }}/{{ $family_update->id }}" enctype="multipart/form-data">
   @else
...
@if(Gate::check('Update Corporate Supplementary Barcode') )
...
// $(document).ready(function() {
        ...
        sProcessing: "<img src='{{ url('assets/images/geargif.gif') }}'>"
          ...
          order: [[ 1, 'asc' ]],
          "ajax": {
          'url': '{{ route('co-familymember.datatable',$membershipdata->id) }}',
          'type': 'POST',
          'headers': {
