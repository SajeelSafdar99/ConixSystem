...
<ul class="breadcrumbee mg-b-25  border-bottom-custom">
  ...
  <li><a href>Edit Corporate Membership</a></li>
</ul>
@else
<ul class="breadcrumbee mg-b-25  border-bottom-custom">
  <li><a href="{{ url('/') }}">Home</a></li>
  <li><a href="{{ url('club-hospitality') }}">Club Membership Management</a></li>
  <li><a href="{{ url('club-hospitality/corporate-membership-vue') }}">Corporate Memberships List</a></li>
  <li><a href>Add Corporate Membership</a></li>
</ul>
@endif

<div class="w3-bar w3-black">
  @if($init==1)
   <a href="{{ url('club-hospitality/corporate-membership/corporate-membership-aeu') }}/{{ $membership_update->id }}">
  <button class="w3-bar-item w3-button  w3-red theactiveclass">Corporate Member</button>
</a>
  @else
 <button class="w3-bar-item w3-button w3-red theactiveclass" onclick="location.href='{{ url('club-hospitality/membership/membership-aeu') }}'; ">Corporate Member</button>
  @endif

@can('View Corporate Family Member')
@if($init==1)
  <a href="{{ url('club-hospitality/corporate-membership/corporate-familymember-aeu') }}/{{ $membership_update->id }}">
  <button class="w3-bar-item w3-button  w3-red">Family Member</button>
</a>
@else
 <a href="{{ url('club-hospitality/corporate-membership/corporate-familymember-aeu/'.$id) }}">
...
@can('View Corporate Membership Documents')
@if($init==1)
<a href="{{ url('club-hospitality/corporate-membership/corporate-membership-docs-aeu') }}/{{ $membership_update->id }}">
  <button class="w3-bar-item w3-button  w3-red">Membership Documents</button>
</a>
@else
<a href="{{ url('club-hospitality/corporate-membership/corporate-membership-docs-aeu/'.$id) }}">
...
@can('View Corporate Cards')
  @if($init==1)
<a href="{{ url('corporate-memberprint') }}/{{ $membership_update->id }}" target="_blank">
  <button class="w3-bar-item w3-button  w3-red">Card</button>
</a>
@else
<a href="{{ url('corporate-memberprint/'.$id) }}">
...
@can('View Corporate Cards')
  @if($init==1)
<a href="{{ url('club-hospitality/corporate-membership/familymembercard') }}/{{ $membership_update->id }}">
  <button class="w3-bar-item w3-button  w3-red">Family Cards</button>
</a>
@else
<a href="{{ url('club-hospitality/corporate-membership/familymembercard/'.$id) }}">
...
@endif
              ...
              @if($init==1 && Gate::check('Update Corporate Member Barcode'))
    <form method="post" enctype="multipart/form-data" action="{{ url('club-hospitality/corporate-membership/barcodeupdate') }}/{{ $membership_update->id }}">
      @elseif($init==1 && !Gate::check('Update Corporate Member Barcode'))
  <form method="post" enctype="multipart/form-data" action="{{ url('club-hospitality/corporate-membership/update') }}/{{ $membership_update->id }}">
     @else
...
<br><br>
 ...
 <div class="col-sm-6">
          ...
          <div class="row mg-t-10">
                      ...
                      <div class="row mg-t-10">
                        <label class="col-sm-4 form-control-label">
                            Corporate Company:  <span class="tx-danger">*</span>
                        </label>
                        <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                              <select @if ($errors->has('corporate_company')) style="border-color:red;" @endif id="corporate_company" onchange="corporatecompany(this.value)" class="form-control" name="corporate_company">

                                <option label="Choose Option">
                                </option>
                                @foreach($companies as $t)
                                @if($init==1)
                                 <option @if(old('corporate_company',$membership_update->corporate_company)==$t->id)  selected @endif  value="{{ $t->id }}">
                                    {{ $t->name }}
                                </option>
                                @else
                                 <option @if(old('corporate_company')==$t->id)  selected @endif  value="{{ $t->id }}">
                                    {{ $t->name }}
                                </option>
                                @endif
...
@endif
                  ...
                  &nbsp&nbsp
                  <a href="{{ url('club-hospitality/corporate-membership-vue') }}" class="btn btn-secondary">Cancel</a>
             </div>
              </div>
    </div>
...
var copid =document.getElementById("corporate_company").value;
event.preventDefault();
$.ajax({
type : 'GET',
url : '{{ url('/') }}/club-hospitality/membership/getcategory/'+catid+'/'+copid,
headers: {
...
$('#memship-number').val(xxx.uniquecode+' '+xxx.inc+'-'+xxx.coinc);
