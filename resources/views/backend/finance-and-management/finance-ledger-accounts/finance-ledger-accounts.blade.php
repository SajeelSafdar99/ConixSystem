...
@section('page-content')
    ...
    <div class="br-pagebody">
        <div>
            ...
            @if($roomledgeraccounts==0) Member / Guest Ledgers @else Room Ledger Accounts @endif</h6>
...
@if($roomledgeraccounts==0)
                ...
                <ul class="breadcrumbee border-bottom-custom">
                    ...
                    <li><a href>Room Ledger Accounts List</a></li>
                </ul>
                @endif
  ...
  <div class="col-sm-4 mg-t-10 mg-sm-t-0">
                                    <label class="rdiobox">
                                        <input type="radio" name="mog" value="0" @if(($mog) && $mog==0) checked="checked" @endif ><span class="pabs">Member</span>
                                    </label>
                                </div>
                                <div class="col-sm-4 mg-t-10 mg-sm-t-0">
                                    <label class="rdiobox">
                                        <input type="radio" name="mog" value="1" @if(($mog) && $mog==1) checked="checked" @endif ><span class="pabs">Guest</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select class="form-control" name="filter">
                                    ...
                                    @foreach($types as $type)
                                        <option @if($filter==$type->id) selected="selected" @endif value="{{$type->id}}">{{$type->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<tr style="background-color: #55bff9;">
    ...
    <td colspan="1"  class="text-left balance">{{isset($opening)?number_format($opening):0}}</td>
</tr>
                            ...
                            <tr class="colors-{{$r->debit_or_credit}}">
                                ...
                                <td>
                                    @if($r->debit_or_credit==0)
                                        ...
                                        <a target="_blank" href="{{route($d->type->details,$d->trans_type_id)}}">({{$d->trans_type_id}} - {{$d->type->name}})</a><br>
                                        @endforeach
                                        @else
                                        @foreach($details as $d)
                                            <a target="_blank" href="{{route('cash.receipt.print',$d->receipt_id)}}">({{$d->receipt_id}} - {{$d->type->name}})</a>
                                        @endforeach
                                    @endif
                                </td>
