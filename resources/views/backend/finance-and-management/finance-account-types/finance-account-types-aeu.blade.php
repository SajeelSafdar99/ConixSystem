...
<ul class="breadcrumbee mg-b-25 border-bottom-custom">
 <li><a href="{{ url('/') }}">Home</a></li>
  ...
  <li><a href>Edit Level Five</a></li>
</ul>
@else
<ul class="breadcrumbee mg-b-25 border-bottom-custom">
 <li><a href="{{ url('/') }}">Home</a></li>
...
@endif
    ...
    @if($init==1)
    <form method="post" action="{{ url('finance-and-management/finance-account-types/update') }}/{{ $acc_type_update->id }}">
     @else
    <form method="post">
    @endif     
    @csrf   
              ...
              <div class="form-layout form-layout-4 ">
                ...
                <div class="row">
                 ...
                 <div class="row mg-t-10">
                  ...
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <select @if ($errors->has('level_three')) style="border-color:red;" @endif  id="level_three" name="level_three" class="form-control" onchange="levelthreedependency(this.id)">
                                <option label="Choose Option">
                                </option>
                               
                                @foreach($thirds as $third)
                                @if($init==1)
                                <option @if(old('level_three',$selected_three[0])==$third->id) selected @endif value="{{$third->id}}">
                                    {{$third->desc}}
                                </option>
                                @else
  ...
  <div class="row mg-t-10">
                  <label class="col-sm-4 form-control-label">Level Four: <span class="tx-danger">*</span></label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                     <select @if ($errors->has('desc')) style="border-color:red;" @endif  id="desc" name="desc" class="form-control">
                                <option label="Choose Option">
                                </option>
                               
                                @foreach($account_heads as $head)
                                @if($init==1)
                                <option @if(old('desc',$acc_type_update->desc)==$head->id) selected @endif value="{{$head->id}}">
                                    {{$head->desc}}
                                </option>
                                @else
                                <option @if(old('desc')==$head->id)  selected @endif value="{{ $head->id }}">
                                    {{$head->desc}}
                                </option>
                                @endif
                                @endforeach
                ...
                <div class="row mg-t-10">
                  ...
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
   <input name="type" type="text" class="form-control input-height" placeholder="Enter Name of Level" value="@if($init==0){{old('type')}}@else{{old('type',$acc_type_update->type)}}@endif">
                  </div>
                </div>
    ...
    <div class="row mg-t-10">
                    ...
                    Status: 
                        ...
                        <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                            <select @if ($errors->has('status')) style="border-color:red;" @endif class="form-control" name="status" value="@if($init==0){{old('status')}}@else{{old('status',$acc_type_update->status)}}@endif">

                                @if($init==1)

                                <option @if($init==0) selected="" @else @if(old('status',$acc_type_update->status)=='1') selected @endif @endif value="1">
                                    Active
                                </option>
                                <option @if(old('status',$acc_type_update->status)=='0') selected @endif value="0">
                                    In-Active
                                </option>
...
<div class="row mg-t-10">
                ...
                <div class="form-layout-footer mg-t-30">
                  ...
                  &nbsp&nbsp
                  <a href="{{ url('finance-and-management/finance-account-types') }}" class="btn btn-secondary">Cancel</a>
...
<script type="text/javascript">
  function levelthreedependency(idd){

  var idval=document.getElementById(idd).value;

    $.ajax({
    type : 'GET',
    url : '{{ url('finance-and-management/levelthree/levelfour/') }}/'+idval,
  headers: {
'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
},
  success: function(data){
...
console.log(data);
$('#desc').html('<option label="Choose Option">  </option>');
            $.each(data,function(x,y){
               let s='<option value="'+y.id+'">'+y.desc+'</option>';
 $('#desc').append(s);
                })
