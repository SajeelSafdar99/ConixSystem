...
<div class="br-pagebody">
        <div>
          <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10 hidden-print margara">Receipts</h6>
...
<span class="middiv widthclass"><strong>{{$profiledata->company_name}}</strong> <br>
  ...
  <span class="float-right"> <h1>RECEIPT</h1></span>
...
<div><strong>Name:</strong>&nbsp&nbsp @if($receiptdata->customer) {{$receiptdata->customer->customer_name}} @elseif($receiptdata->employee) {{$receiptdata->employee->name}} @elseif($receiptdata->member) {{$receiptdata->member->title}} {{$receiptdata->member->first_name}} {{$receiptdata->member->middle_name}} {{$receiptdata->member->applicant_name}} @elseif($receiptdata->corporatemember) {{$receiptdata->corporatemember->title}} {{$receiptdata->corporatemember->first_name}} {{$receiptdata->corporatemember->middle_name}} {{$receiptdata->corporatemember->applicant_name}} @endif</div>

<div><strong>Category:</strong>&nbsp&nbsp @if($receiptdata->customer && $receiptdata->customer->guest_type==1) Applied Member @elseif($receiptdata->customer && $receiptdata->customer->guest_type==2) Affiliated Member @elseif($receiptdata->member) Member @elseif($receiptdata->corporatemember) Corporate Member @elseif($receiptdata->employee) Employee @endif</div>

@if($receiptdata->member)
<div><strong>Membership #:</strong>&nbsp&nbsp {{$receiptdata->member->mem_no}}</div>
@elseif($receiptdata->corporatemember)
<div><strong>Corporate Membership #:</strong>&nbsp&nbsp {{$receiptdata->corporatemember->mem_no}}</div>
...
<div><strong>Contact #:</strong>&nbsp&nbsp @if($receiptdata->customer) {{$receiptdata->customer->customer_contact}} @elseif($receiptdata->employee) {{$receiptdata->employee->mob_a}} @elseif($receiptdata->member) {{$receiptdata->member->mob_a}} @elseif($receiptdata->corporatemember) {{$receiptdata->corporatemember->mob_a}} @endif</div>
...
@elseif($receiptdata->corporatemember)
<div><strong>City:</strong>&nbsp&nbsp @if($receiptdata->corporatemember->cur_city){{$receiptdata->corporatemember->cur_city}}@endif </div>


<div><strong>Family Member:</strong>&nbsp&nbsp{{copinvoicesfamilyname($receiptdata->family)}} @if($receiptdata->family)(@endif{{familyrelationship(copinvoicesfamilyrelation($receiptdata->family))}} @if($receiptdata->family))@endif</div>
...
@php
    ...
    $invoice=\App\finance_invoice::find($var_b->trans_type_id);
@endphp
<td class="left">{{$divide}}</td>



 @if($var_b->trans_type==1) 
<td class="left"><?php echo '<a target="_blank" href="'.route(transTypesDetails($var_b->trans_type),$var_b->trans_type_id).'">'.$var_b->trans_type_id.'</a>' ?> ({{transTypesChargesTypes($var_b->trans_type)}}) {{formatDateToShow($var_b->roomBookingAttribute->check_in_date)}} - {{formatDateToShow($var_b->roomBookingAttribute->check_out_date)}} @if($invoice && $invoice->familyD && $receiptdata->member) {{$invoice->familyD->name}} ({{$invoice->familyD->relationship_name->desc}}) @elseif($invoice && $invoice->cofamilyD && $receiptdata->corporatemember) {{$invoice->cofamilyD->name}} ({{$invoice->cofamilyD->relationship_name->desc}}) @endif</td>

@else
<td class="left"><?php echo '<a target="_blank" href="'.route(transTypesDetails($var_b->trans_type),$var_b->trans_type_id).'">'.$var_b->trans_type_id.'</a>' ?> ({{transTypesChargesTypes($var_b->trans_type)}}) @if($invoice && $invoice->start_date) {{formatDateToShow($invoice->start_date)}}@endif - @if($invoice && $invoice->end_date) {{formatDateToShow($invoice->end_date)}}@endif @if($invoice && $invoice->familyD && $receiptdata->member)
    {{$invoice->familyD->name}} ({{$invoice->familyD->relationship_name->desc}}) @elseif($invoice && $invoice->cofamilyD && $receiptdata->corporatemember) {{$invoice->cofamilyD->name}} ({{$invoice->cofamilyD->relationship_name->desc}}) @endif</td>
...
<div class="row" style="margin-top: -20px !important;">
  ...
  <p style="font-size: 12px !important"><strong>RECEIVED BY:</strong>&nbsp<u style="text-transform: uppercase;">@if($receiptdata->created_by){{usernames($receiptdata->created_by)}}@endif</u></p>
</div>
