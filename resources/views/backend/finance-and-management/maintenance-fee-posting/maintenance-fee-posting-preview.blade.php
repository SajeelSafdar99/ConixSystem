...
<div class="br-pagebody">
        <div>
         ...
         <div style="text-align: right;" style="text-align: right; margin-top: -39px;">
...
<div class="col-sm-6 col-sm-offset-3">
                      <h6 class="box-title headingsettings"><b>CHARGES For <span class="count"></span> Members </b></h6><br>
<i class="fa fa-arrow-circle-left fa-3x" onclick="prev()"></i>
<i class="fa fa-arrow-circle-right fa-3x"   onclick="next()"></i>
...
<br><br><br> <br><br>
                                            ...
                                            <span  ><span  class="count"></span> out of <span class="total"></span> are done </span>
                                                <div class="progress">
...
@push('jscode')
    <script>
        let kik=0
        let data={!! json_encode($r) !!};
        function next(){
            if(kik<data.length-1){
                kik++
                showD(data[kik]);
            }
        ...
        function showD(x){
            $('#member_id').val(x['member_id'])
            $('#mem_no').val(x['mem_no'])
            $('#invoice_date').val(x['invoice_date'])
            $('#name').val(x['name'])
            $('#statusx').val(x['active'].desc)
            $('[name="charges_type[]"]').val(x['charges_type'][0])
            $('[name="charges_amount[]"]').val(x['total_maintenance'])
            $('[name="per_day_amount[]"]').val(((x['total_maintenance']/30)*100)/100)
            $('[name="start_date[]"]').val(x['start_date'][0])
            $('[name="end_date[]"]').val(x['end_date'][0])
            $('[name="days[]"]').val(x['days'][0])
            $('[name="qty[]"]').val(x['qty'][0])
            $('[name="total[]"]').val(x['total'][0])
            $('[name="final_total"]').val(x['final_total'])
            $('[name="discount_amount"]').val(x['discount_amount'])
            $('[name="discount_percentage"]').val(x['discount_percentage'])
            $('[name="discount_details"]').val(x['discount_details'])
            $('[name="extra_charges"]').val(x['extra_charges'])
            $('[name="extra_percentage"]').val(x['extra_percentage'])
            $('[name="tax_charges"]').val(x['tax_charges'])
            $('[name="tax_percentage"]').val(x['tax_percentage'])
            $('[name="tax_details"]').val(x['tax_details'])
            $('[name="grand_total"]').val(x['grand_total'])
            $('[name="amount_in_words"]').val(x['amount_in_words'])
            $('[name="comments"]').val(x['comments'])
            $('.status').prop('checked',false)
            x['status'].forEach((a)=>{
                console.log('.status[value="'+a+'"]');
                $('.status[value="'+a+'"]').prop('checked',true)
