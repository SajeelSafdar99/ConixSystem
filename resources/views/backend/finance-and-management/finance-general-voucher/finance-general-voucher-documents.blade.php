...
<div class="br-pagebody">
          ...
          <h5 class="tx-gray-800 tx-uppercase tx-bold tx-18 mg-b-10" style="text-align: center;">GENERAL VOUCHER DOCUMENTS<br>(VOUCHER #: {{$receiptdata->invoice_no}})</h5>
...
@if($receiptdata->invoice_type==0)

    @foreach($receiptdata->generalVoucherDocs->pluck('url') as $image)
     <a href="{{url($image)}}" target="_blank">
                    <img src="{{ url($image) }}"  height="450" width="400" >
                    </a>
                    @endforeach

@elseif($receiptdata->invoice_type==2)
...
@elseif($receiptdata->invoice_type==3)
    ...
    @foreach($receiptdata->generalVoucherDocs->pluck('url') as $image)
     <a href="{{url($image)}}" target="_blank">
                    ...
                    @elseif($receiptdata->invoice_type==4)
...
@elseif($receiptdata->invoice_type==1)
