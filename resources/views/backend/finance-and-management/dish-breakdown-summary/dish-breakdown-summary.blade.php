...
.select2-selection__placeholder {
  color: #c4c4c4 !important; 
    ...
    <div class="br-pagebody">
        <div>
            <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10  hidden-print">Dish Breakdown Summary (Price)</h6>
 ...
 <ul class="breadcrumbee border-bottom-custom">
    ...
    <li><a href="{{ url('finance-and-management/reports/dish-breakdown-summary') }}">Dish Breakdown Summary List</a></li>
    </ul>
...
<div class="col-md-4">
  @if($chosensubcat!=[])
                           ...
                           <select id="subsearch" name="subsearch[]" class="form-control js-example-basic-multiple" multiple="multiple">
                             @foreach($subcatsearch as $subs)
                             
                             <option value="{{$subs->id}}" @if(in_array($subs->id,$chosensubcat)) selected="selected" @endif>{{$subs->desc}}</option>
  ...
  @elseif($chosensubcat==[])
                           ...
                           <select id="subsearch" name="subsearch[]" class="form-control js-example-basic-multiple" multiple="multiple">
                             @foreach($subcatsearch as $subs)
                             <option value="{{$subs->id}}">{{$subs->desc}}</option>
                             @endforeach
                           </select>
...
<div style="text-align: center; color: black; letter-spacing: 0.2em !important;">
  <h3>DISH BREAKDOWN SUMMARY (PRICE)</h3>
<hr>
</div>
...
@foreach($subcategories as $subcat)
                      @if($subcat->item_category == $category->id)

  @foreach($itemdefinitions as $item)
                           @if($item->sub_category == $subcat->id)  
...
@endforeach
    ...
    @php $catexist = 0; @endphp
                      @foreach($subcategories as $subcat)
                      @if($subcat->item_category == $category->id)
@php $catexist = 1; @endphp
                      @endif
                      @endforeach
     ...
     <div class="header" style="text-transform: uppercase; "><b>{{$category->desc}}</b></div> 
                ...
                <thead>
                        <tr>
                            ...
                            <th class="wd-5p">Net Sale Value</th>
                        </tr>
                        </thead>
                      </table>
...
@foreach($itemdefinitions as $item)
 @foreach($salessubs as $sub)
 @if($sub->saleid &&  $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
@if($item->sub_category == $subcat->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)
    @php $thevar= 1; @endphp
@endif
@endif
@endforeach
...
<!-- CHECKING IF THE SUB CATEGORY HEADING SHOULD APPEAR OR NOT -->
 ...
 <h5 class="tx-uppercase tx-bold" style="color: black;"><u>{{$subcat->desc}}</u> :</h5>
...
@endforeach 
         ...
         <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($item->sale_price);
                                @endphp" class="wd-5p">{{$item->sale_price}}</td>

          @php $sum1 = 0; @endphp
         @foreach($salessubs as $sub)
         @if($sub->saleid &&  $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
      @if(is_numeric($sub->sub_total_price) && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $sum1+= $sub->sub_total_price; @endphp
        @endif    @endif      
@endforeach 
       <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($sum1);
                                @endphp" class="wd-5p">{{number_format($sum1)}}</td>


         @php $sum2 = 0; @endphp
         @foreach($salessubs as $sub)
         @if($sub->saleid &&  $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
      @if(is_numeric($sub->item_discount) && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $sum2+= $sub->item_discount; @endphp
        @php $rounded_disc=round($sum2); @endphp
        @endif      @endif        
@endforeach 
       <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($rounded_disc);
                                @endphp" class="wd-5p">{{number_format($sum2)}}</td>

        @php $sum3 = 0; @endphp
         @foreach($salessubs as $sub)
          @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
      @if(is_numeric($sub->total) && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $sum3+= $sub->total; @endphp
        @php $rounded_total=round($sum3); @endphp
        @endif       @endif       
@endforeach 
       <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($rounded_total);
                                @endphp" class="wd-5p">{{number_format($sum3)}}</td>
  ...
  @php $i++; @endphp
    ...
    <td colspan="5" class="text-right">{{$subcat->desc}} Total:</td>
   ...
   @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
         @if($subcat->item_category == $category->id)
      @if(is_numeric($sub->sub_total_price) && $item->sub_category == $subcat->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $subtotal+= $sub->sub_total_price; @endphp
...
@endforeach @endforeach 
    ...
    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($subtotal);
                                @endphp" class="text-left">{{number_format($subtotal)}}</td>

     @php $itemdiscount = 0; @endphp
     @foreach($itemdefinitions as $item)
         @foreach($salessubs as $sub)
         @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
         @if($subcat->item_category == $category->id)
      @if(is_numeric($sub->item_discount) && $item->sub_category == $subcat->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $itemdiscount+= $sub->item_discount; @endphp
        @php $grand_rounded_disc=round($itemdiscount); @endphp
        @endif   
         @endif        @endif 
@endforeach @endforeach

    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($grand_rounded_disc);
                                @endphp" class="text-left">{{number_format($itemdiscount)}}</td>
     ...
     @php $total = 0; @endphp
      @foreach($itemdefinitions as $item)
         ...
         @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
         @if($subcat->item_category == $category->id)
      @if(is_numeric($sub->total) && $item->sub_category == $subcat->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $total+= $sub->total; @endphp
       @php $grand_rounded_total=round($total); @endphp
        @endif   
         @endif        @endif 
@endforeach @endforeach 
    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($grand_rounded_total);
                                @endphp" class="text-left">{{number_format($total)}}</td>
</tr>
...
<table  class="table display nowrap datatable">
  <tbody>
                            ...
                            <tr style="background-color: #1cdad1;">
                              <td class="wd-5p" ></td>
                           <td class="wd-5p" ></td>
    <td class="wd-15p text-right">{{$category->desc}} Total:</td>
         ...
         @foreach($salessubs as $sub)
  @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
   @if(is_numeric($sub->sub_total_price) && $item->category == $category->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)
...
@endforeach 
    ...
    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($subtotal);
                                @endphp" class="wd-5p">{{number_format($subtotal)}}</td>

     @php $itemdiscount = 0; @endphp
       @foreach($itemdefinitions as $item)
         @foreach($salessubs as $sub)
    @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
  @if(is_numeric($sub->item_discount) && $item->category == $category->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $itemdiscount+= $sub->item_discount; @endphp
        @php $final_rounded_disc=round($itemdiscount); @endphp
        @endif   
         @endif
           @endforeach     
@endforeach 
    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($final_rounded_disc);
                                @endphp" class="wd-5p">{{number_format($itemdiscount)}}</td>

     @php $total = 0; @endphp
       @foreach($itemdefinitions as $item)
         @foreach($salessubs as $sub)
    @if($sub->saleid && $sub->saleid->date >= formatDate($start_date) && $sub->saleid->date <= formatDate($end_date))
  @if(is_numeric($sub->total) && $item->category == $category->id && $sub->item_code == $item->item_code && $sub->item_details == $item->item_details)

        @php $total+= $sub->total; @endphp
         @php $final_rounded_total=round($total); @endphp
        @endif   
         @endif
            @endforeach  
@endforeach 
    <td title="@php $x=new NumberFormatter('en',NumberFormatter::SPELLOUT);
                                    echo $x->format($final_rounded_total);
                                @endphp" class="wd-5p">{{number_format($total)}}</td>
</tr>
  </tbody>
</table>
