...
<div class="">
<br>
<!-- copy($("#addmoreid tr:first-child")[0]) -->
<script type="text/javascript">
  let cl='<tr>\n' +
      '                                                            <td>\n' +
      '                                                              <select onchange="expenseselect(this.id)" id="expense_head" class="form-control input-height select2" name="expense_head[]">\n' +
      ...
      '</td>\n' +
      '  <td>\n' +
      '                                                              <select id="expense_payable" class="form-control input-height select2" name="expense_payable[]">\n' +
      '    <option label="Choose Option"></option>\n' +
      '                                                                                                  <option value="19">\n' +
      '                                  Laundry\n' +
      '                                </option>\n' +
      ...
      '                                                            <td>\n' +
      '\n' +
      '<input type="text" id="expense_details" name="expense_details[]" class="form-control input-height" autocomplete="off" value="" placeholder="Enter Details">\n' +
      '                                                            \n' +
      '                                                            </td>\n' +
      '                                                            <td>\n' +
      '                                                              <input onkeyup="chargesselect2()" type="number" id="charges" name="charges[]" class="form-control input-height charamt" autocomplete="off" value="" placeholder="Enter Amount">\n' +
      '                                                              \n' +
      '                                                            </td>\n' +
      '\n' +
      '                                                            \n' +
   ...
   <table align="center"  style="border:2px solid deepskyblue;" width="100%">
                                                    <tbody>
                                                    <tr>
                                                        <td width="40%" align="left">Expense Head</td>
                                                        <td width="20%" align="left">Expense Paid For</td>
                                                        <td width="20%" align="left">Expense Details</td>
                                                        <td width="15%" align="left">Charges</td>
                                                    ...
                                                    @else
                                                        ...
                                                        <tr>
                                                            <td>
                                                              <select onchange="expenseselect(this.id)" @if ($errors->has('expense_head')) style="border-color:red;" @endif  id="expense_head" class="form-control input-height select2" name="expense_head[]" >
    <option label="Choose Option"></option>
 @foreach($expense_heads as $head)
  ...
  <td>
                                                              <select @if ($errors->has('expense_payable')) style="border-color:red;" @endif  id="expense_payable" class="form-control input-height select2" name="expense_payable[]" >
    <option label="Choose Option"></option>
 @foreach($expense_payables as $payable)
 @can($payable->name.' '.$payable->mod_id)
                              @if($init==1)
                                 <option @if(old('expense_payable[]',$expenses_update->expense_payable)==$payable->id)  selected @endif  value="{{ $payable->id }}">
                                  {{ $payable->name }}
                                </option>
                                @else
                                 <option @if(old('expense_payable[]')==$payable->id)  selected @endif value="{{ $payable->id }}">
                                  {{ $payable->name }}
                                </option>
                                @endif
                            @endcan
...
<input @if ($errors->has('expense_details')) style="border-color:red;" @endif type="text" id="expense_details" name="expense_details[]" class="form-control input-height" autocomplete="off" value="@if($init==0){{old('expense_details[]')}}@else{{old('expense_details[]',$expenses_update->expense_details)}}@endif" placeholder="Enter Details">
                                                            ...
                                                            <td>
                                                              <input onkeyup="chargesselect2()" @if ($errors->has('charges')) style="border-color:red;" @endif type="number" id="charges" name="charges[]" class="form-control input-height charamt" autocomplete="off" value="@if($init==0){{old('charges[]')}}@else{{old('charges[]',$expenses_update->charges)}}@endif" placeholder="Enter Amount">

                                                            </td>


                                                            <td>
                                                                <button type="button" class="btn btn-danger btn-xs" onclick="$(this).parents('tr').remove()"><i class="fa fa-minus-circle"></i> </button>
                                                            </td>
  ...
  <div class="row mg-t-10">
              ...
              <label class="col-sm-1 form-control-label">
                                ...
                                *
                                                     ...
                                                     <div class="col-sm-4 mg-t-10 mg-sm-t-0">
         <input @if ($errors->has('total_amount')) style="border-color:red;" @endif type="number" id="total_amount" name="total_amount" class="form-control input-height" autocomplete="off" oninput="add_surcharge()" value="@if($init==0){{old('total_amount')}}@else{{old('total_amount',$expenses_update->total_amount)}}@endif" readonly style="background-color: #c1c1c1">
</div>
...
ss.find('#expense_payable').html('<option label="Choose Option">  </option>');
            $.each(data,function(x,y){
               let s='<option value="'+y.id+'">'+y.name+'</option>';
ss.find('#expense_payable').append(s);
                })
