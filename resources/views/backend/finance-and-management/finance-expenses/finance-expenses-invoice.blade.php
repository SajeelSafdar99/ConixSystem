...
p {  color: black !important;  font: 120% sans-serif !important;}
h2 { text-align: center !important; font: bold 200% sans-serif !important; padding-right: 220px;}
/* table */


h1 { font: bold 120% sans-serif !important; letter-spacing: 0.2em !important; text-transform: uppercase !important; color: black !important; padding-top: 27px !important; }
...
<div class="br-pagebody">
        <div>
          <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10 hidden-print margara">Expense Vouchers</h6>
...
<ul class="breadcrumbee mg-b-25  border-bottom-custom">
  ...
  <li><a href>Print Expense Voucher</a></li>
</ul>
...
<span class="middiv widthclass"><strong>{{$profiledata->company_name}}</strong> <br>
  ...
  <span class="float-right"> <h1>EXPENSE<br> VOUCHER</h1></span>
...
<div><strong>Name:</strong>&nbsp&nbsp {{$receiptdata->person_name}}</div>
<div><strong>Supplier #:</strong>&nbsp&nbsp {{$receiptdata->person_id}}</div>
<div><strong>CNIC #:</strong>&nbsp&nbsp @if($receiptdata->ledgerperson){{$receiptdata->ledgerperson->person_cnic}}@endif</div>
<div><strong>Contact #:</strong>&nbsp&nbsp{{$receiptdata->person_contact}}</div>
<div><strong>Email:</strong>&nbsp&nbsp @if($receiptdata->ledgerperson){{$receiptdata->ledgerperson->person_email}}@endif</div>
<div><strong>Address:</strong>&nbsp&nbsp @if($receiptdata->ledgerperson){{$receiptdata->ledgerperson->person_address}}@endif</div>
...
<th class="left">Expense Paid For</th>
<th class="center">Expense Details</th>
<th class="right">Amount Paid in Words</th>
</tr>
</thead>
...
<td class="left">{{$expense_payables->name}}</td>

<td class="center">{{$receiptdata->expense_details}}</td>
<td class="right">{{$receiptdata->amount_in_words}}</td>
...
<td class="right">@if($receiptdata->surcharge==NULL) 0 @else {{$receiptdata->surcharge}} @endif</td>
</tr>
