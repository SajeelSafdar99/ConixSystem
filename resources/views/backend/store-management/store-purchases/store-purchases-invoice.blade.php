...
<div class="br-pagebody">
        <div>
          <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10 hidden-print margara">Store Purchases</h6>
...
<ul class="breadcrumbee mg-b-25  border-bottom-custom">
  ...
  <li><a href="{{ url('store-management/store-purchases-vue') }}">Purchases List</a></li>
...
<span class="middiv widthclass"><strong>{{$profiledata->company_name}}</strong> <br>
  ...
  <span class="float-right"> <h1>GOOD RECEIPT NOTE
  {{-- @if($receiptdata->approved==1) PURCHASE<br>INVOICE @else GOOD RECEIPT NOTE @endif --}}
  {{-- <br><br>@if($resultant) (unpaid) @elseif(!$resultant) (paid) @endif --}}
  </h1></span>
...
<div class="col-sm-2">
<h5 class="mb-3"><strong>PURCHASED FROM:</strong>
</h5>
</div>

<div class="col-sm-4">
<div><strong>Name:</strong>&nbsp&nbsp @if($receiptdata->ledgerperson){{$receiptdata->ledgerperson->person_name}}@endif</div>
<div><strong>Supplier #:</strong>&nbsp&nbsp @if($receiptdata->ledgerperson){{$receiptdata->ledgerperson->id}}@endif</div>
...
@if(explode('.', $sub->purchase_price)[1]==0)
  <td class="text-left">{{ round($sub->purchase_price) }}</td>
@else
  <td class="text-left">{{ $sub->purchase_price }}</td>
@endif
...
<div class="row" >
  ...
  <p style="font-size: 12px !important">Store Manager:&nbsp_______________</p>
</div>
