...
<div class="w3-bar w3-black">
 ...
 <button class="w3-bar-item w3-button w3-red " onclick="location.href='{{ url('human-resource/employment/employment-aeu/') }}/{{Request::segment(4)}}' ">Employee</button>
  ...
  <button class="w3-bar-item w3-button  w3-red theactiveclass" onclick="location.href='{{ url('human-resource/employment/reference-aeu/') }}/{{Request::segment(4)}}' ">References</button>
...
<div style="text-align: right !important; margin-top: -25px;" class="col-sm-8 mg-t-10 mg-sm-t-0">
          ...
          <a href="{{ url('human-resource/reference/deleted') }}/{{$employee_data->id}}">
...
{{-- Education Modal --}}
@if($init==1)
<form method="post" action="{{ url('human-resource/reference/update/') }}/{{ $ref_update->employee_id }}/{{ $ref_update->id }}" >
   @else
    <form method="post" >
    @endif
    @csrf
            ...
            <div class="modal-dialog modal-dialog-vertical-center" role="document" style="width: 1000px;">
              <div class="modal-content bd-0 tx-14">
                <div class="modal-header pd-y-20 pd-x-25">
                  <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Add Reference</h6>
...
<div id="error_msg" class="col-sm-6 col-sm-offset-3 alert alert-success text-center">{{$errors->first()}}
      ...
      @endif
                ...
                <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                   <input @if ($errors->has('name')) style="border-color:red;" @endif  id="name" type="text" name="name" class="form-control input-height" placeholder="Enter Full Name" value="@if($init==0){{old('name')}}@else{{old('name',$ref_update->name)}}@endif">
              </div>
            </div>
                ...
                <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                   <input @if ($errors->has('address')) style="border-color:red;" @endif id="address" type="text" name="address" class="form-control input-height" placeholder="Enter Complete Address" value="@if($init==0){{old('address')}}@else{{old('address',$ref_update->address)}}@endif">
                </div>
                  </div>
   ...
   <div class="row mg-t-10">
                        ...
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <input @if ($errors->has('contact')) style="border-color:red;" @endif id="contact" type="text" name="contact" class="form-control input-height" placeholder="Enter Contact Number" value="@if($init==0){{old('contact')}}@else{{old('contact',$ref_update->contact)}}@endif">
                        </div>
                    </div>
  ...
  <div class="row mg-t-10">
                        ...
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                          <input @if ($errors->has('years')) style="border-color:red;" @endif id="years" class="form-control input-height" placeholder="Enter Number of Years" value="@if($init==0){{old('years')}}@else{{old('years',$ref_update->years)}}@endif"  type="number" name="years">
...
// $(document).ready(function() {
        ...
        sProcessing: "<img src='{{ url('assets/images/geargif.gif') }}'>"
          ...
          order: [[ 1, 'asc' ]],
          "ajax": {
          'url': '{{ route('reference.datatable',$employee_data->id) }}',
          'type': 'POST',
          'headers': {
