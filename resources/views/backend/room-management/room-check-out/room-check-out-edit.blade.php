...
<br>
 ...
 @if($init==1)   <div class="col-sm-1 mg-t-10 mg-sm-t-0">
    ...
    <input @if($init==0) checked="" @else @if(old('booking_type',$roombooking->booking_type)=='0') checked="" @endif @endif type="radio" name="booking_type" value="0"><span class="pabs">Member</span>
      ...
      <label class="rdiobox">
    <input @if($init==0) checked="" @else @if(old('booking_type',$roombooking->booking_type)=='6') checked="" @endif @endif type="radio" name="booking_type" value="6"><span class="pabs">Corporate Member</span>
              </label>
            </div><!-- col-3 -->
              ...
              @foreach($gts as $gt)
                ...
                <label class="rdiobox">
    <input @if(old('booking_type',customertype($roombooking->customer_id))==$gt->id) checked="" @endif type="radio" name="booking_type" value="{{10+$gt->id}}" data-errormessage-value-missing="Guest Type is Required !" class="validate[required]"><span class="pabs">{{$gt->desc}}</span>
              </label>
...
<!-- 
    ...
    <input @if($init==0) checked="" @else @if(old('booking_type')=='0') checked="" @endif @endif type="radio" name="booking_type" value="0"><span class="pabs">Member</span></label>
              ...
              @foreach($gts as $gt)
                ...
                <label class="rdiobox">
    <input @if(old('booking_type')==$gt->id) checked="" @endif type="radio" name="booking_type" value="{{10+$gt->id}}" data-errormessage-value-missing="Guest Type is Required !" class="validate[required]"><span class="pabs">{{$gt->desc}}</span>
              </label>
...
<!-- @if($init==1){{$roombooking->food_bill_charges}}@endif -->
                        </div>
                    </div>
        ...
        function customerdata(val) {
            ...
            $.ajax({
                ...
                $lname=val1.applicant_name?val1.applicant_name:'';
                       ...
                       let name = v == 0 ? $fname+$mname+$lname : val1.customer_name ;
                        let code = v == 0 ? val1.mem_no : val1.customer_no;
                        let status = v == 0 ? '('+val1.mem_status.desc+')' : ''; 
